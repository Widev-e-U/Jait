...
export function detectInteractivePrompt(output) {
    if (!output)
        return false;
 ...
 */
async function ensureSessionTerminal(registry, context, preferredId) {
    ...
    if (preferredId) {
        try {
            ...
            if (s && s.type === "terminal" && s.state === "running") {
                return { surface: s, terminalId: preferredId, isNew: false };
            }
        }
    ...
    if (existingId) {
        try {
            ...
            if (s && s.type === "terminal" && s.state === "running") {
                return { surface: s, terminalId: existingId, isNew: false };
            }
        }
        catch { /* gone */ }
 ...
 */
function executeInTerminal(surface, command, timeoutMs, onChunk, signal) {
    return new Promise((resolve) => {
        ...
        const finish = (timedOut, exitCode = null) => {
            if (settled)
                return;
            settled = true;
            clearTimeout(timer);
...
export function createTerminalRunTool(registry, sandboxManager = new SandboxManager(), ws) {
    return {
        name: "terminal.run",
        description: "Execute a shell command in a persistent terminal (visible to the user) and return the output. " +
        ...
        async execute(input, context) {
            ...
            try {
                ...
                const { surface, terminalId, isNew, warning } = await ensureSessionTerminal(registry, context, preferredId);
                // 2. Execute the command (sentinel-based)
                const execPromise = executeInTerminal(surface, command, timeout, context.onOutputChunk, context.signal);
                ...
                if (isNew)
                    message += ` [new terminal ${terminalId}]`;
                if (warning)
                    message += ` [${warning}]`;
                if (needsInteraction) {
                    ws?.sendUICommand({
                ...
                console.log(`[terminal.run] ${message}; output (${result.output.length} chars): ${JSON.stringify(result.output.slice(0, 500))}`);
                return {
                    ok,
                    message,
                    data: {
...
export function createTerminalStreamTool(registry) {
    return {
