...
export function createThreadControlTool(deps) {
    ...
    const ensureUserId = (context) => {
        const userId = context.userId?.trim();
        return userId ? userId : undefined;
    };
    const getAccessibleThread = (threadId, userId) => {
    ...
    const startThread = async (thread, message, attachments) => {
        if (thread.status === "running") {
            return { ok: false, message: "Thread is already running" };
        }
        const provider = deps.providerRegistry.get(thread.providerId);
        if (!provider) {
    ...
    return {
        ...
        parameters: {
            ...
            properties: {
                action: {
                    type: "string",
                    description: "Action: list, get, create, create_many, update, delete, start, send, stop, interrupt, approve, activities, create_pr.",
                    enum: [
                        "list",
                        "get",
                        "create",
        ...
        async execute(input, context) {
            ...
            try {
                switch (input.action) {
                    ...
                    case "get": {
                        if (!input.threadId)
                            return { ok: false, message: "get requires `threadId`." };
                    ...
                    case "create_many": {
                        ...
                        const startResults = await Promise.all(startTargets.map(async ({ thread, spec }) => {
                            const started = await startThread(thread, spec.message ?? input.message, spec.attachments ?? input.attachments);
                            return {
                                threadId: thread.id,
                                ok: started.ok,
                    ...
                    case "update": {
                        if (!input.threadId)
                            return { ok: false, message: "update requires `threadId`." };
                        ...
                        const thread = deps.threadService.update(input.threadId, {
                            ...
                            prState: input.prState === "open" || input.prState === "closed" || input.prState === "merged"
                                ? input.prState
                                : input.prState === null
                                    ? null
                                    : undefined,
                        ...
                        if (!thread)
                            return { ok: false, message: "Thread not found after update." };
                        broadcastThreadEvent(thread.id, "updated", { thread });
                        return { ok: true, message: "Thread updated", data: { thread } };
                    }
                    case "delete": {
                        if (!input.threadId)
                            return { ok: false, message: "delete requires `threadId`." };
                        const thread = getAccessibleThread(input.threadId, userId);
                        if (!thread)
                            return { ok: false, message: "Thread not found." };
                    ...
                    case "stop": {
                        if (!input.threadId)
                            return { ok: false, message: "stop requires `threadId`." };
                        const thread = getAccessibleThread(input.threadId, userId);
                        if (!thread)
                            return { ok: false, message: "Thread not found." };
                    ...
                    case "interrupt": {
                        if (!input.threadId)
                            return { ok: false, message: "interrupt requires `threadId`." };
                        const thread = getAccessibleThread(input.threadId, userId);
                        if (!thread)
                            return { ok: false, message: "Thread not found." };
                        if (!thread.providerSessionId)
                    ...
                    case "approve": {
                        if (!input.threadId)
                            return { ok: false, message: "approve requires `threadId`." };
                        if (!input.requestId?.trim())
                            return { ok: false, message: "approve requires `requestId`." };
                        const thread = getAccessibleThread(input.threadId, userId);
                        if (!thread)
                            return { ok: false, message: "Thread not found." };
                        if (!thread.providerSessionId)
                    ...
                    case "activities": {
                        if (!input.threadId)
                            return { ok: false, message: "activities requires `threadId`." };
                        const thread = getAccessibleThread(input.threadId, userId);
                        if (!thread)
                            return { ok: false, message: "Thread not found." };
                    ...
                    case "create_pr": {
                        ...
                        if (!cwd)
                            return { ok: false, message: "create_pr requires `cwd` or a thread with `workingDirectory`." };
                        const action = input.gitAction ?? "commit_push_pr";
                        const result = await gitService.runStackedAction(cwd, action, input.commitMessage?.trim() || undefined, input.featureBranch === true, input.baseBranch);
                        ...
                        if (thread) {
                            ...
                            if (prUrl) {
                                const activity = deps.threadService.addActivity(thread.id, "activity", result.pr.url ? `Pull request created: ${prUrl}` : `Open pull request: ${prUrl}`, { action, prUrl, result });
                                broadcastThreadEvent(thread.id, "activity", { activity });
                            }
                        }
                    ...
                    default:
                        return { ok: false, message: `Unsupported action: ${input.action ?? "(unknown)"}` };
                }
            }
            catch (err) {
                return {
