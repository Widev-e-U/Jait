...
function isSonnet4(endpoint: ModelEndpoint): boolean {
  const m = endpoint.model.toLowerCase();
  return m === "claude-sonnet-4" || m === "claude-sonnet-4-20250514";
}

function isClaude45(endpoint: ModelEndpoint): boolean {
  const m = endpoint.model.toLowerCase();
...
function sonnet4Prompt(mode: ChatMode): string {
...
function claude45Prompt(mode: ChatMode): string {
  const modeBlock = getModeInstructions(mode);
  return `<instructions>
${CORE_INSTRUCTIONS}
</instructions>
...
function claude46Prompt(mode: ChatMode): string {
  const modeBlock = getModeInstructions(mode);
  return `<instructions>
${CORE_INSTRUCTIONS}
...
function anthropicReminder(_mode: ChatMode, _endpoint: ModelEndpoint): string {
...
const AnthropicPromptResolver: IAgentPrompt = {
  name: "anthropic",
  familyPrefixes: ["claude", "Anthropic"],

  resolveSystemPrompt(mode, endpoint) {
    if (isSonnet4(endpoint)) return sonnet4Prompt(mode);
    if (isClaude45(endpoint)) return claude45Prompt(mode);
