...
For every finite one-shot command — including builds, tests, installs, OCR, downloads, and scripts — wait synchronously for completion. Use \`timeout: 0\` when it may run longer than 30 seconds. Use \`isBackground: true\` only for indefinite processes such as servers, watchers, and daemons; never poll or send another command to that watched terminal.
...
Wait for finite one-shot commands to complete, using \`timeout: 0\` when needed. Use \`isBackground: true\` only for indefinite servers, watchers, or daemons; never poll or reuse their watched terminal.
