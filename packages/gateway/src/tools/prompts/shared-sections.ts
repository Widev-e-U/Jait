...
Keep command output small: scope commands to explicit paths with \`--include\`/\`--exclude\` and never grep whole repos, build output, dist/web-dist bundles, source maps (\`.map\`), lockfiles, or node_modules. Bound unknown result sizes with \`-m N\` or \`| head\`; if a command is noisy by nature, redirect it to a file and grep that file instead. Oversized output is collapsed per line and truncated before you see it, so a flood hides the useful matches.
