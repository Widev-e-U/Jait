...
export function validateToolInput(schema, input) {
    ...
    for (const [key, propSchema] of Object.entries(schema.properties)) {
        ...
        if (checker && !checker(value)) {
            ...
            errors.push(`Property '${key}' expected type '${propSchema.type}', got '${typeof value}'`);
        }
        // Enum check
        if (propSchema.enum && propSchema.enum.length > 0) {
            const current = obj[key]; // may have been coerced
            if (!propSchema.enum.includes(String(current))) {
                errors.push(`Property '${key}' must be one of [${propSchema.enum.join(", ")}], got '${current}'`);
            }
        }
    }
