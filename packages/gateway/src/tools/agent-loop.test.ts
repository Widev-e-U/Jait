...
describe("runAgentLoop incomplete narration recovery", () => {
  ...
  it("continues when a model stops after only promising the next action", async () => {
    const fetchSpy = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValueOnce(sseResponse("Next, I’ll add the watch-options UI, then run the tests."))
      .mockResolvedValueOnce(sseResponse("Implemented the UI and all focused tests pass."));
    ...
    const result = await runAgentLoop(
      {
        ...
        history: [
          { role: "system", content: "system" },
          { role: "user", content: "Finish the implementation and test it." },
        ],
        toolSchemas: [],
        hasTools: true,
        sessionId: "session-1",
    ...
    expect(fetchSpy).toHaveBeenCalledTimes(2);
    expect(result.content).toContain("Implemented the UI and all focused tests pass.");
  });
});

describe("runAgentLoop todo continuity", () => {
  function sseToolResponse(callId: string, name: string, args: unknown): Response {
    return new Response(
      new ReadableStream<Uint8Array>({
        start(controller) {
          const encoder = new TextEncoder();
  ...
  it("requires a todo refresh before finishing after additional work", async () => {
    const initialItems = [
      { id: 1, title: "Implement feature", status: "in-progress" as const },
      { id: 2, title: "Run tests", status: "not-started" as const },
    ];
    const completedItems = initialItems.map((item) => ({ ...item, status: "completed" as const }));
    const fetchSpy = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValueOnce(sseToolResponse("todo-1", "todo", { todoList: initialItems }))
      .mockResolvedValueOnce(sseToolResponse("execute-1", "execute", { command: "run tests" }))
      .mockResolvedValueOnce(sseTextResponse("Implementation finished."))
      .mockResolvedValueOnce(sseToolResponse("todo-2", "todo", { todoList: completedItems }))
      .mockResolvedValueOnce(sseTextResponse("Implementation and tests are complete."));
    ...
    const result = await runAgentLoop(
      {
        ...
        history: [
          { role: "system", content: "system" },
          { role: "user", content: "Implement and test the feature." },
        ],
        toolSchemas: [],
        hasTools: true,
        sessionId: "todo-continuity-session",
        abort: new AbortController(),
        maxRounds: 6,
        mode: "agent",
      },
      async (name, args) => name === "todo"
        ? { ok: true, message: "Todo updated", data: { items: (args as { todoList: unknown[] }).todoList } }
        : { ok: true, message: "Command completed" },
    );

    expect(fetchSpy).toHaveBeenCalledTimes(5);
    expect(result.content).toContain("Implementation and tests are complete.");
  });
});
