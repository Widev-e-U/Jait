 ...
 */
export function createJaitTool(deps) {
    return {
        name: "jait",
        description: "Jait platform tool — access memory, scheduler, network, and gateway status.\n\n" +
            "**Actions:**\n" +
        ...
        parameters: {
            ...
            properties: {
                action: {
                    type: "string",
                    description: "The action to perform: memory.save, memory.search, memory.forget, " +
            ...
            try {
                switch (input.action) {
                    ...
                    case "memory.search": {
                        ...
                        const results = await deps.memoryService.search(input.query, input.limit ?? 5, input.scope);
                    ...
                    case "cron.update": {
                        ...
                        const updated = deps.scheduler.update(input.jobId, {
                            name: input.name,
                            cron: input.cron,
