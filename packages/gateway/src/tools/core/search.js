...
export function createSearchTool(registry) {
    return {
        name: "search",
        ...
        parameters: {
            ...
            properties: {
                pattern: {
                    type: "string",
                    description: "The pattern to search for. Use regex with alternation (e.g. 'word1|word2') or character " +
                        "classes to find multiple words in a single search. Set `isRegexp` appropriately.",
                },
                isRegexp: {
                    type: "boolean",
                    description: "Whether the pattern is a regex (default: false). Search is case-insensitive by default.",
                },
                path: {
                    type: "string",
                ...
                include: {
                    type: "string",
                    description: 'File glob to filter results (e.g. "*.ts", "src/**/*.py"). ' +
                        "Applied to paths relative to the search directory.",
                },
                limit: {
                    type: "number",
                    description: `Maximum number of results (default: ${DEFAULT_MAX_RESULTS}, max: ${MAX_RESULTS_CAP}). ` +
                        "Don't set this unless necessary — it can slow things down.",
                },
                includeIgnoredFiles: {
                    type: "boolean",
                    description: "Whether to include files normally ignored by .gitignore and search.exclude settings. " +
        ...
        async execute(input, context) {
            ...
            try {
                ...
                const result = await searchContent(searchDir, input.pattern, input.include, limit, input.isRegexp ?? false, input.includeIgnoredFiles ?? false);
                // Auto-retry with opposite mode if no results (Copilot pattern)
                if (result.ok && result.data?.matches?.length === 0) {
                    const retryResult = await searchContent(searchDir, input.pattern, input.include, limit, !(input.isRegexp ?? false), input.includeIgnoredFiles ?? false);
                    if (retryResult.data?.matches?.length > 0) {
                        return {
                            ...retryResult,
...
async function searchContent(dir, pattern, include, limit, isRegexp, includeIgnored) {
...
async function searchFiles(dir, pattern, limit) {
    const isWin = platform() === "win32";
    const safeDir = dir.replace(/"/g, '\\"');
