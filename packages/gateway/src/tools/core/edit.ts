...
export function createEditTool(registry: SurfaceRegistry): ToolDefinition<EditInput> {
  return {
    name: "edit",
    description:
      "Create a new file, overwrite an existing file, or patch (search-and-replace) part of a file.\n\n" +
    ...
    parameters: {
      ...
      properties: {
        ...
        search: {
          type: "string",
          description:
            "The exact literal text to find in the file (for patch mode). " +
        ...
        replace: {
          type: "string",
          description:
            "The exact literal replacement text (for patch mode). " +
