 ...
 */
async function deriveWorkspaceRoot(targetPath) {
 ...
 */
function surfaceCovers(surface, targetPath) {
 ...
 */
function pickMostSpecific(surfaces, targetPath) {
    let best = null;
    let bestLen = -1;
    for (const s of surfaces) {
        if (!surfaceCovers(s, targetPath))
            continue;
        const root = s.snapshot().metadata?.workspaceRoot;
 ...
 */
export function resolveWorkspaceRoot(registry, sessionId, 
...
//# sourceMappingURL=get-fs.js.map