...
const STT_PROVIDER_VALUES = new Set(["simulated", "browser"]);
const CHAT_PROVIDER_VALUES = new Set(["jait", "codex", "claude-code"]);
function asRecord(value) {
    if (!value || typeof value !== "object")
        return null;
    return value;
}
function sanitizeApiKeys(input) {
    const out = {};
...
export function registerAuthRoutes(app, config, users, toolRegistry) {
    ...
    app.patch("/auth/settings", async (request, reply) => {
        ...
        const body = request.body ?? {};
        const patch = {};
        if (typeof body.theme === "string" && THEME_VALUES.has(body.theme)) {
        ...
        if (Array.isArray(body.disabled_tools)) {
            patch.disabledTools = body.disabled_tools.filter((v) => typeof v === "string");
        }
        if (typeof body.stt_provider === "string" && STT_PROVIDER_VALUES.has(body.stt_provider)) {
            patch.sttProvider = body.stt_provider;
        }
        if (typeof body.chat_provider === "string" && CHAT_PROVIDER_VALUES.has(body.chat_provider)) {
            patch.chatProvider = body.chat_provider;
        }
    ...
    app.patch("/auth/settings/tools", async (request, reply) => {
        ...
        for (const name of disabledToolsInput) {
            ...
            if (coreTools.has(name))
                continue; // can't disable core tools
            filtered.push(name);
        }
