...
export function registerNetworkRoutes(app) {
    ...
    app.post("/api/network/scan", async (request) => {
        ...
        const targetSubnet = body["subnet"];
        ...
        const hosts = await Promise.all(subnetFiltered.map(async (entry) => {
            const portResults = await Promise.all(PROBE_PORTS.map(async (port) => ({ port, open: await probePort(entry.ip, port, 1500) })));
    ...
    app.post("/api/network/ssh/test", async (request) => {
        ...
        if (reachable) {
            ...
            try {
                ...
                if (banner)
                    result.platform = banner.trim().slice(0, 100);
            }
            catch {
                // ignore
            }
    ...
    app.post("/api/network/nodes/register", async (request) => {
        ...
        if (!node.id || !node.ip)
            return { error: "id and ip are required" };
