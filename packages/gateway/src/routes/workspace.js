 ...
 */
function findFsSurface(registry, surfaceId) {
    if (surfaceId) {
        const s = registry.getSurface(surfaceId);
        if (s && s instanceof FileSystemSurface && s.state === "running")
            return s;
        return null;
    }
...
export function registerWorkspaceRoutes(app, surfaceRegistry, sessionState, sessionService) {
    ...
    app.get("/api/workspace/info", async (_req, reply) => {
        ...
        return {
            surfaceId: snap.id,
            workspaceRoot: snap.metadata?.workspaceRoot ?? null,
            state: snap.state,
        };
    });
    ...
    app.get("/api/workspace/list", async (req, reply) => {
        const { path: dirPath, surfaceId } = req.query;
        ...
        try {
            const snap = fs.snapshot();
            const root = snap.metadata?.workspaceRoot;
    ...
    app.get("/api/workspace/read", async (req, reply) => {
        const { path: filePath, surfaceId } = req.query;
        const fs = findFsSurface(surfaceRegistry, surfaceId);
        if (!fs) {
            return reply.status(404).send({ error: "NO_WORKSPACE", message: "No filesystem surface is running" });
        }
    ...
    app.get("/api/workspace/stat", async (req, reply) => {
        const { path: targetPath, surfaceId } = req.query;
        const fs = findFsSurface(surfaceRegistry, surfaceId);
        if (!fs) {
            return reply.status(404).send({ error: "NO_WORKSPACE", message: "No filesystem surface is running" });
        }
    ...
    app.post("/api/workspace/apply-diff", async (req, reply) => {
        ...
        try {
            ...
            const isValidationError = err instanceof PathTraversalError || (err instanceof Error && (message.includes("outside workspace root")
