...
async function main() {
    ...
    surfaceRegistry.onSurfaceStarted = (id, surface) => {
        if (surface.type === "terminal" && "write" in surface) {
            surface.onOutput = (data) => ws.broadcastTerminalOutput(id, data);
        }
        if (surface.type === "filesystem") {
            const snap = surface.snapshot();
            const sid = snap.sessionId ?? "";
            const workspaceRoot = snap.metadata?.workspaceRoot ?? null;
    ...
    const sessionApprovalsBySession = new Map();
    const getSessionApprovals = (sessionId) => {
    ...
    const toolExecutor = async (toolName, input, context, options) => {
    ...
    let shutdownFn;
    const shutdownRef = async () => { if (shutdownFn)
        await shutdownFn();
    else
        process.exit(0); };
    ...
    // Seed built-in "Network Scan" job if it doesn't already exist
    {
        ...
        const hasNetworkScan = existingJobs.some((j) => j.toolName === "network.scan" && j.name === "Network Scan");
        if (!hasNetworkScan) {
            // Remove legacy "Device Discovery" job if present
            const legacy = existingJobs.find((j) => j.toolName === "network.scan" && j.name === "Device Discovery");
            if (legacy)
                scheduler.remove(legacy.id);
            scheduler.create({
                name: "Network Scan",
                ...
                input: {
                    __jaitJobMeta: {
                        jobType: "system_job",
                        description: "Scans the local network for devices, checks SSH connectivity, and detects running Jait gateway nodes.",
                    },
                },
                enabled: true,
            });
    ...
    ws.onTerminalReplay = (terminalId) => {
        try {
            ...
            if (surface && surface.type === "terminal" && "getRecentOutput" in surface) {
                return surface.getRecentOutput();
            }
        }
        catch {
            // ignore
