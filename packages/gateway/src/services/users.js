...
export class UserService {
    ...
    verifyCredentials(username, password) {
        ...
        if (!row)
            return null;
        if (!verifyPasswordHash(password, row.passwordHash))
            return null;
        return {
            id: row.id,
            username: row.username,
    ...
    getSettings(userId) {
        ...
        return {
            ...
            disabledTools: parseStringArray(row.disabledTools ?? null),
            sttProvider: row.sttProvider || "simulated",
            chatProvider: row.chatProvider || "jait",
            updatedAt: row.updatedAt,
        };
    }
