...
import { inferSharedRepositories, threadBelongsToRepository, } from '@/lib/automation-repositories';
...
function dbRepoToLocal(repo, localDeviceId) {
    return {
        id: repo.id,
        name: repo.name,
...
export function useAutomation(enabled = true) {
    ...
    const [threadPrStates, setThreadPrStates] = useState({});
    ...
    const sharedRepositories = useMemo(() => inferSharedRepositories(threads, localRepositories), [threads, localRepositories]);
    const repositories = useMemo(() => [...localRepositories, ...sharedRepositories], [localRepositories, sharedRepositories]);
    ...
    const handleThreadEvent = useCallback((eventType, payload) => {
        switch (eventType) {
            case 'thread.created': {
            ...
            case 'thread.activity': {
                ...
                if (threadId && activity) {
                    const nextActivities = mergeActivities(activityCacheRef.current.get(threadId) ?? [], [activity]);
            ...
            case 'repo.updated': {
                const repo = payload.repo;
                if (repo) {
                    setLocalRepositories(prev => prev.map(r => r.id === repo.id ? dbRepoToLocal(repo, localDeviceId) : r));
                }
                break;
            }
            case 'repo.deleted': {
    ...
    useEffect(() => {
        ...
        const baseStates = Object.fromEntries(currentThreads.map((t) => [t.id, null]));
        ...
        const loadPrStates = async () => {
            ...
            const settled = await Promise.allSettled(threadsWithBranch.map(async (thread) => {
                ...
                const status = await gitApi.status(statusCwd, thread.branch ?? undefined);
                ...
                if (status.pr && (thread.prUrl !== status.pr.url ||
    ...
    // ── Thread lifecycle ───────────────────────────────────────────
    const handleSend = useCallback(async (text, providerId = 'jait', model) => {
        if (!text.trim())
            return;
