...
function extractTaggedRepoName(title) {
    const match = title.match(/^\[([^\]]+)\]/);
    return match?.[1]?.trim() || null;
}
function extractWorktreeRepoName(path) {
    if (!path)
        return null;
    const normalized = normalizePath(path);
...
export function inferThreadRepositoryName(thread) {
    return (extractTaggedRepoName(thread.title) ??
        extractWorktreeRepoName(thread.workingDirectory) ??
        (thread.workingDirectory ? folderName(thread.workingDirectory) : null));
}
export function threadBelongsToRepository(thread, repository) {
...
export function inferSharedRepositories(threads, localRepositories) {
...
//# sourceMappingURL=automation-repositories.js.map