 ...
 */
export function activitiesToMessages(activities) {
    const messages = [];
    let current = null;
    ...
    for (const act of activities) {
        ...
        switch (act.kind) {
            ...
            case 'tool.start': {
                const msg = ensureAssistant(act.id);
                const callId = payload.callId ?? act.id;
            ...
            case 'tool.output': {
                ...
                if (callId && toolCallMap.has(callId)) {
                    ...
                    tc.streamingOutput = (tc.streamingOutput ?? '') + (payload.content ?? '');
                }
                break;
            }
            case 'tool.result':
            case 'tool.error': {
                ...
                if (callId && toolCallMap.has(callId)) {
                    const tc = toolCallMap.get(callId);
                    tc.status = payload.ok ? 'success' : 'error';
                    tc.result = {
                        ok: payload.ok ?? false,
                        message: payload.message ?? act.summary,
                    };
            ...
            case 'tool.approval': {
                const msg = ensureAssistant(act.id);
                const callId = payload.requestId ?? act.id;
                const tc = {
                    callId,
                    tool: payload.tool ?? 'unknown',
                    args: payload.args ?? {},
...
//# sourceMappingURL=activity-to-messages.js.map