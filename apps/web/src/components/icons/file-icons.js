 ...
 */
import { getIconForFile, getIconForFolder, getIconForOpenFolder, DEFAULT_FILE, DEFAULT_FOLDER, DEFAULT_FOLDER_OPENED, } from 'vscode-icons-js';
...
export function FileIcon({ filename, className, }) {
    ...
    return (<img src={`${CDN}${icon}`} alt="" className={className} loading="lazy" decoding="async" draggable={false}/>);
}
/**
 * Renders a folder icon (closed or open) for a given folder name.
 */
export function FolderIcon({ name, open = false, className, }) {
    const icon = open
