...
// ── Component ────────────────────────────────────────────────────────
export function ToolSettings({ token }) {
    ...
    const toggleTool = useCallback(async (toolName, enabled) => {
        if (!token)
            return;
        // Optimistic update
    ...
    return (<Card className="p-5 space-y-4">
      ...
      <div className="relative">
        ...
        <Input placeholder="Filter tools..." value={filter} onChange={(e) => setFilter(e.target.value)} className="pl-9"/>
      </div>
      ...
      <div className="space-y-5">
            ...
            return (<div key={category}>
              ...
              <div className="space-y-1">
                {items.map((tool) => {
                    ...
                    return (<div key={tool.name} className="flex items-center gap-3 py-2 px-3 rounded-md hover:bg-muted/50 transition-colors">
                      <Switch checked={tool.enabled} onCheckedChange={(checked) => { void toggleTool(tool.name, checked); }} disabled={tool.locked || saving} aria-label={`Toggle ${tool.name}`}/>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-sm font-medium">{tool.name}</span>
                          <Badge variant={tierInfo.variant} className="text-[10px] px-1.5 py-0">
                            {tierInfo.label}
                          </Badge>
                          {tool.source === 'mcp' && (<Badge variant="outline" className="text-[10px] px-1.5 py-0">
                              MCP
                            </Badge>)}
                        </div>
        ...
        {sortedCategories.length === 0 && (<p className="text-sm text-muted-foreground text-center py-4">
