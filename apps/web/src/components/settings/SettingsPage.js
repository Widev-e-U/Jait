...
export function SettingsPage({ username, token, apiKeys, onSaveApiKeys, sttProvider, onSttProviderChange, onClearArchive, activityEvents, }) {
    ...
    const testGatewayUrl = useCallback(async (url) => {
        if (!url.trim()) {
            // Reset to default
    ...
    function renderSourceBadge(field) {
    ...
    return (<div className="mx-auto w-full max-w-5xl p-6 space-y-6">
      <div>
      ...
      <Card className="p-5 space-y-4">
        ...
        <div className="max-w-md space-y-2">
          ...
          <div className="flex gap-2">
            <Input id="gateway-url" type="url" value={gwDraft} onChange={(e) => { setGwDraft(e.target.value); setGwStatus('idle'); }} placeholder={getApiUrl()} className="font-mono text-sm"/>
            <Button variant="outline" onClick={() => { void testGatewayUrl(gwDraft); }} disabled={gwStatus === 'testing'} className="shrink-0">
              {gwStatus === 'testing' ? (<Loader2 className="h-4 w-4 animate-spin"/>) : 'Test & save'}
            </Button>
          </div>
          {gwStatus === 'ok' && (<p className="text-sm text-green-600 dark:text-green-400 flex items-center gap-1">
              <CheckCircle2 className="h-3.5 w-3.5"/>
              {gwDraft.trim() ? 'Connected — gateway URL saved.' : 'Reset to default.'}
            </p>)}
          {gwStatus === 'error' && gwError && (<p className="text-sm text-destructive flex items-center gap-1">
              <AlertCircle className="h-3.5 w-3.5"/>
              {gwError}
            </p>)}
          <p className="text-xs text-muted-foreground">
            Current: <code className="bg-muted px-1 py-0.5 rounded text-[11px]">{getApiUrl()}</code>
            {getStoredGatewayUrl() && (<button type="button" className="ml-2 text-xs underline text-muted-foreground hover:text-foreground" onClick={() => { setGwDraft(''); setStoredGatewayUrl(null); setGwStatus('idle'); }}>
                Reset to default
              </button>)}
          </p>
        </div>
      ...
      <Card className="p-5 space-y-4">
        ...
        <div className="max-w-sm">
          ...
          <Select value={sttProvider} onValueChange={(value) => { void onSttProviderChange(value); }}>
          ...
          {API_KEY_FIELDS.map((field) => {
            ...
            return (<div key={field} className="space-y-1.5">
                <div className="flex items-center gap-1.5">
                  {IconComponent ? (<IconComponent size={14} className="shrink-0 text-muted-foreground"/>) : (<Key className="h-3.5 w-3.5 shrink-0 text-muted-foreground"/>)}
                  <Label htmlFor={`api-${field}`} className="font-mono text-xs">{field}</Label>
                  {renderSourceBadge(field)}
                </div>
                <div className="relative">
                  <Input id={`api-${field}`} type={secret && !shown ? 'password' : 'text'} value={draft[field] ?? ''} onChange={(event) => {
                    const next = event.target.value;
                    setDraft((prev) => ({ ...prev, [field]: next }));
                }} placeholder={envSet[field] ? '(set via .env)' : '(empty)'} className={secret ? 'pr-9' : ''}/>
                  {secret && (<button type="button" className="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors" onClick={() => toggleVisibility(field)} tabIndex={-1} aria-label={shown ? 'Hide value' : 'Show value'}>
      ...
      {activityEvents && activityEvents.length > 0 && (<Card className="p-5 space-y-4">
          <div>
