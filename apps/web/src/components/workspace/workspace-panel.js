...
export function workspaceLanguageForPath(path) {
...
async function remoteScanDir(dirPath, surfaceId) {
    let url = `${API_URL}/api/workspace/list?path=${encodeURIComponent(dirPath)}`;
    if (surfaceId)
        url += `&surfaceId=${encodeURIComponent(surfaceId)}`;
...
async function remoteReadFile(filePath, surfaceId) {
...
async function remoteStatFile(filePath, surfaceId) {
...
/*  Drag resize hook                                                   */
/* ------------------------------------------------------------------ */
function useDragResize(initial, min, max, direction = 'horizontal') {
...
/*  Tree node component                                                */
/* ------------------------------------------------------------------ */
function TreeNodeRow({ node, depth, activeFilePath, expandedDirs, onToggleDir, onSelectFile, onContextFile, }) {
    const paddingLeft = 8 + depth * 14;
    if (node.kind === 'dir') {
        const expanded = expandedDirs.has(node.path);
        const loading = node.childrenLoading;
        return (<>
        <div className="group flex items-center gap-1 rounded px-1 py-1 cursor-pointer text-xs hover:bg-muted" style={{ paddingLeft }} onClick={() => onToggleDir(node)}>
          {loading ? (<Loader2 className="h-3 w-3 shrink-0 text-muted-foreground animate-spin"/>) : (<ChevronRight className={`h-3 w-3 shrink-0 text-muted-foreground transition-transform duration-150 ${expanded ? 'rotate-90' : ''}`}/>)}
          <FolderIcon name={node.name} open={expanded} className="h-3.5 w-3.5 shrink-0"/>
          <span className="truncate flex-1">{node.name}</span>
        </div>
        {expanded && node.children?.map((child) => (<TreeNodeRow key={child.path} node={child} depth={depth + 1} activeFilePath={activeFilePath} expandedDirs={expandedDirs} onToggleDir={onToggleDir} onSelectFile={onSelectFile} onContextFile={onContextFile}/>))}
      </>);
    }
    const isActive = activeFilePath === node.path;
    return (<div className={`group flex items-center gap-1 rounded px-1 py-1 cursor-pointer text-xs ${isActive ? 'bg-primary/15 text-foreground' : 'hover:bg-muted'}`} style={{ paddingLeft: paddingLeft + 14 }} onClick={() => onSelectFile(node)} onContextMenu={(e) => { e.preventDefault(); onContextFile(node); }} draggable onDragStart={(e) => {
      ...
      <button type="button" className="opacity-0 group-hover:opacity-100 p-0.5 rounded hover:bg-background" onClick={(e) => { e.stopPropagation(); onContextFile(node); }} title="Add to chat">
...
/*  WorkspacePanel                                                     */
/* ------------------------------------------------------------------ */
export const WorkspacePanel = forwardRef(function WorkspacePanel({ files, activeFileId, onActiveFileChange, onFileDrop: _onFileDrop, onReferenceFile, onAvailableFilesChange, autoOpenRemotePath, surfaceId, isMobile, showTree: showTreeProp = true, showEditor: showEditorProp = true, onToggleTree, onToggleEditor, changedPaths, }, ref) {
    const rootDirHandle = useRef(null);
    /** When non-null, we're in remote (server-backed) mode */
    const [remoteRoot, setRemoteRoot] = useState(null);
    ...
    // Lazy tree state
    const [lazyTree, setLazyTree] = useState([]);
    const [expandedDirs, setExpandedDirs] = useState(() => new Set());
    const [activeNativePath, setActiveNativePath] = useState(null);
    ...
    const prevChangedPathsRef = useRef(new Set());
    useEffect(() => {
    ...
    const editorFile = activeNativePath
        ...
        : activeExtFile
            ...
            : null;
    /* ---- Open directory ---- */
    const handleOpenDirectory = useCallback(async (handle) => {
        let root = handle;
        if (!root) {
    ...
    const handleOpenRemoteWorkspace = useCallback(async (rootPath) => {
        try {
    ...
    useEffect(() => {
        ...
        if (remoteRoot === autoOpenRemotePath)
            return;
    ...
    const handleReadFileByPath = useCallback(async (path) => {
    ...
    /* ---- Select native file ---- */
    const handleSelectNativeFile = useCallback(async (node) => {
        onActiveFileChange('');
    ...
    /* ---- Context / reference ---- */
    const handleContextNativeFile = useCallback(async (node) => {
        try {
            const content = node.handle ? await readFileHandle(node.handle) : await remoteReadFile(node.path, surfaceId);
    ...
    const handleSelectExtFile = useCallback((id) => {
        setActiveNativePath(null);
        setPreviewContent(null);
    ...
    const handleSelectNativeFileMobile = useCallback(async (node) => {
        await handleSelectNativeFile(node);
        if (isMobile)
            setMobileTab('editor');
    }, [handleSelectNativeFile, isMobile]);
    const handleSelectExtFileMobile = useCallback((id) => {
    ...
    if (isMobile) {
        ...
        if (!showTreeProp && !showEditorProp)
            return null;
        return (<div className="flex flex-col h-full min-h-0">
        {/* Tab bar — only show when both tabs are available */}
        {showTreeProp && showEditorProp && (<div className="flex items-center h-8 border-b bg-muted/30 shrink-0 px-1 gap-0.5">
          <button className={`flex items-center gap-1 px-2.5 py-1 rounded text-[11px] font-medium transition-colors ${effectiveMobileTab === 'files' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground'}`} onClick={() => setMobileTab('files')}>
            <FolderOpen className="h-3 w-3"/>
            Files
          </button>
          <button className={`flex items-center gap-1 px-2.5 py-1 rounded text-[11px] font-medium transition-colors ${effectiveMobileTab === 'editor' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground'}`} onClick={() => setMobileTab('editor')}>
            Editor
            {editorFile && (<span className="text-[10px] text-muted-foreground truncate max-w-[120px]">
        ...
        {!(showTreeProp && showEditorProp) && (<div className="flex items-center justify-between h-8 border-b bg-muted/30 shrink-0 px-2">
        ...
        {effectiveMobileTab === 'files' && showTreeProp && (<>
          ...
          <div className="flex items-center justify-end h-7 px-2 shrink-0 border-b bg-muted/10">
            {onToggleTree && (<button onClick={onToggleTree} className="flex items-center gap-1 text-[11px] text-muted-foreground hover:text-foreground transition-colors rounded px-1.5 py-0.5 hover:bg-muted">
                <EyeOff className="h-3 w-3"/>
              </button>)}
          </div>
          <ScrollArea className="flex-1 min-h-0">
            <div className="p-1">
              {hasNativeTree && lazyTree.map((node) => (<TreeNodeRow key={node.path} node={node} depth={0} activeFilePath={activeNativePath} expandedDirs={expandedDirs} onToggleDir={handleToggleDir} onSelectFile={handleSelectNativeFileMobile} onContextFile={handleContextNativeFile}/>))}

              {hasExtFiles && (<>
                  {hasNativeTree && (<div className="text-[10px] uppercase tracking-wider text-muted-foreground px-2 pt-3 pb-1 font-medium">
                      Dropped files
                    </div>)}
                  {files.map((file) => (<div key={file.id} onClick={() => handleSelectExtFileMobile(file.id)} className={`group flex items-center gap-1 rounded px-1 py-1.5 cursor-pointer text-xs ${!activeNativePath && activeFileId === file.id ? 'bg-primary/15 text-foreground' : 'hover:bg-muted active:bg-muted'}`} style={{ paddingLeft: 22 }}>
                      <FileIcon filename={file.name} className="h-3.5 w-3.5"/>
                      <span className="truncate flex-1" title={file.path}>{file.path}</span>
                    </div>))}
                </>)}

              {!hasNativeTree && !hasExtFiles && (<div className="text-xs text-muted-foreground p-3 text-center">
        ...
        {/* Editor tab */}
        {effectiveMobileTab === 'editor' && showEditorProp && (<div className="flex-1 flex flex-col min-h-0">
            <div className="flex items-center gap-1.5 h-7 px-2 border-b bg-muted/20 shrink-0">
              {editorFile && showTreeProp && (<button className="flex items-center gap-1 text-[11px] text-muted-foreground hover:text-foreground" onClick={() => setMobileTab('files')}>
              ...
              {onToggleEditor && (<button onClick={onToggleEditor} className="flex items-center gap-1 text-[11px] text-muted-foreground hover:text-foreground transition-colors rounded px-1.5 py-0.5 hover:bg-muted ml-auto">
                  <EyeOff className="h-3 w-3"/>
                </button>)}
            </div>
            <div className="flex-1 min-h-0 overflow-auto">
              {loadingFile ? (<div className="h-full flex items-center justify-center text-sm text-muted-foreground gap-2">
                  <Loader2 className="h-4 w-4 animate-spin"/>
                  Loading...
                </div>) : editorFile ? (<pre className="text-[11px] leading-relaxed p-2 font-mono whitespace-pre overflow-x-auto text-foreground">
                  <code>{editorFile.content}</code>
                </pre>) : (<div className="h-full flex items-center justify-center text-xs text-muted-foreground px-4 text-center">
    ...
    /* ---- Desktop layout ---- */
    return (<aside className="border-r bg-muted/20 flex min-h-0 shrink-0" style={{ width: !showTreeProp && !showEditorProp ? 0 : !showTreeProp ? Math.max(panel.size - tree.size, 300) : !showEditorProp ? tree.size + 8 : panel.size, maxWidth: '70vw' }}>
      {/* File explorer pane */}
      {showTreeProp && (<div className={`border-r bg-background transition-colors flex flex-col shrink-0`} style={{ width: tree.size }}>
        ...
        <div className="flex items-center justify-between h-7 px-2 border-b bg-muted/20 shrink-0">
          ...
          {onToggleTree && (<button onClick={onToggleTree} className="flex items-center gap-1 text-[11px] text-muted-foreground hover:text-foreground transition-colors rounded px-1 py-0.5 hover:bg-muted">
              <EyeOff className="h-3 w-3"/>
              Hide
            </button>)}
        </div>
        <ScrollArea className="flex-1 min-h-0">
          <div className="p-1">
            {hasNativeTree && lazyTree.map((node) => (<TreeNodeRow key={node.path} node={node} depth={0} activeFilePath={activeNativePath} expandedDirs={expandedDirs} onToggleDir={handleToggleDir} onSelectFile={handleSelectNativeFile} onContextFile={handleContextNativeFile}/>))}

            {hasExtFiles && (<>
                {hasNativeTree && (<div className="text-[10px] uppercase tracking-wider text-muted-foreground px-2 pt-3 pb-1 font-medium">
                    Dropped files
                  </div>)}
                {files.map((file) => (<div key={file.id} onClick={() => handleSelectExtFile(file.id)} onContextMenu={(e) => { e.preventDefault(); onReferenceFile(file); }} className={`group flex items-center gap-1 rounded px-1 py-1 cursor-pointer text-xs ${!activeNativePath && activeFileId === file.id ? 'bg-primary/15 text-foreground' : 'hover:bg-muted'}`} style={{ paddingLeft: 22 }}>
                    <FileIcon filename={file.name} className="h-3.5 w-3.5"/>
                    <span className="truncate flex-1" title={file.path}>{file.path}</span>
                    <button type="button" className="opacity-0 group-hover:opacity-100 p-0.5 rounded hover:bg-background" onClick={(e) => { e.stopPropagation(); onReferenceFile(file); }} title="Add to chat">
                      <Send className="h-3 w-3"/>
                    </button>
                  </div>))}
              </>)}

            {!hasNativeTree && !hasExtFiles && (<div className="text-xs text-muted-foreground p-2">No files loaded yet.</div>)}
          </div>
        </ScrollArea>
      ...
      {showTreeProp && showEditorProp && (<div className="w-1 shrink-0 cursor-col-resize hover:bg-primary/20 active:bg-primary/30 transition-colors" onMouseDown={tree.onMouseDown}/>)}

      {/* Editor pane */}
      {showEditorProp && (<div className="flex-1 min-w-0 min-h-0 flex flex-col">
        {/* Editor header with hide button */}
        <div className="flex items-center justify-between h-7 px-2 border-b bg-muted/20 shrink-0">
          <span className="text-[11px] text-muted-foreground truncate">
            {editorFile ? editorFile.path.split('/').pop() : 'Editor'}
          </span>
          {onToggleEditor && (<button onClick={onToggleEditor} className="flex items-center gap-1 text-[11px] text-muted-foreground hover:text-foreground transition-colors rounded px-1 py-0.5 hover:bg-muted">
              <EyeOff className="h-3 w-3"/>
              Hide
            </button>)}
        </div>
        ...
        {loadingFile ? (<div className="h-full flex items-center justify-center text-sm text-muted-foreground gap-2">
            <Loader2 className="h-4 w-4 animate-spin"/>
            Loading...
          </div>) : editorFile ? (<Editor height="100%" theme="vs-dark" path={editorFile.path} language={editorFile.language} value={editorFile.content} options={{
                    readOnly: true,
      ...
      <div className="w-1 shrink-0 cursor-col-resize hover:bg-primary/20 active:bg-primary/30 transition-colors" onMouseDown={panel.onMouseDown}/>
    </aside>);
});
//# sourceMappingURL=workspace-panel.js.map