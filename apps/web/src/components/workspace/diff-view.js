 ...
 */
function computeMergedContent(originalLines, modifiedLines, hunks) {
    ...
    for (let i = hunks.length - 1; i >= 0; i--) {
        const h = hunks[i];
        if (h.state === 'rejected')
            continue; // keep original — nothing to do
        ...
        if (h.modifiedEndLineNumber === 0) {
            ...
            newLines = modifiedLines.slice(h.modifiedStartLineNumber - 1, h.modifiedEndLineNumber);
        }
...
/*  Component                                                          */
/* ------------------------------------------------------------------ */
export function DiffView({ filePath, originalContent, modifiedContent, language, onClose, onApply, }) {
    ...
    useEffect(() => {
        if (!editorRef.current || hunks.length === 0)
            return;
    ...
    /* ---- Hunk decorations ---- */
    useEffect(() => {
        if (!editorRef.current || !monacoRef.current || hunks.length === 0)
            return;
        ...
        return () => {
            try {
                modEditor.deltaDecorations(ids, []);
            }
            catch { /* unmounted */ }
        };
    }, [hunks]);
    ...
    /* ---- Actions ---- */
    const setHunkState = useCallback((index, state) => {
        setHunks(prev => {
    ...
    const acceptAll = useCallback(() => {
        setHunks(prev => prev.map(h => ({ ...h, state: 'accepted' })));
    }, []);
    const rejectAll = useCallback(() => {
        setHunks(prev => prev.map(h => ({ ...h, state: 'rejected' })));
    }, []);
    ...
    return (<div className="flex flex-col h-full bg-background">
      ...
      <div className="flex items-center justify-between h-10 px-3 border-b bg-muted/30 shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {hunks.length > 0 && (<span className="text-[10px] text-muted-foreground shrink-0">
              {hunks.length} change{hunks.length !== 1 ? 's' : ''}
              {undecidedCount > 0 && (<span className="text-amber-500 ml-1">{undecidedCount} pending</span>)}
            </span>)}
        </div>

        <div className="flex items-center gap-1 shrink-0">
          <Button size="sm" variant="ghost" className="h-7 px-2 text-[11px]" onClick={acceptAll} disabled={undecidedCount === 0}>
            <Check className="h-3 w-3 mr-1"/>
            Accept all
          </Button>
          <Button size="sm" variant="ghost" className="h-7 px-2 text-[11px]" onClick={rejectAll} disabled={undecidedCount === 0}>
      ...
      <div className="flex-1 min-h-0">
        <DiffEditor original={originalContent} modified={modifiedContent} language={language} theme="vs-dark" onMount={handleMount} options={{
            readOnly: true,
      ...
      {hunks.length > 0 && (<div className="flex items-center justify-between h-9 px-3 border-t bg-muted/20 shrink-0">
          {/* Hunk counter & navigation */}
          <div className="flex items-center gap-1">
            <Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={goPrev} disabled={activeIndex === 0}>
              <ChevronLeft className="h-3.5 w-3.5"/>
            </Button>
            <span className="text-[11px] text-muted-foreground min-w-[60px] text-center">
              Change {activeIndex + 1} of {hunks.length}
            </span>
            <Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={goNext} disabled={activeIndex === hunks.length - 1}>
          ...
          <div className="flex items-center gap-2">
            {activeHunk && activeHunk.state !== 'undecided' && (<span className={cn('text-[10px] font-medium', activeHunk.state === 'accepted' ? 'text-green-500' : 'text-red-500')}>
                {activeHunk.state === 'accepted' ? 'Kept' : 'Reverted'}
              </span>)}
            {activeHunk && activeHunk.state === 'undecided' && (<>
                <Button size="sm" variant="ghost" className="h-6 px-2 text-[11px] text-green-600 hover:text-green-500 hover:bg-green-500/10" onClick={acceptHunk}>
                  <Check className="h-3 w-3 mr-1"/>
                  Keep
                </Button>
                <Button size="sm" variant="ghost" className="h-6 px-2 text-[11px] text-red-600 hover:text-red-500 hover:bg-red-500/10" onClick={rejectHunk}>
                  <Undo2 className="h-3 w-3 mr-1"/>
                  Undo
                </Button>
              </>)}
          ...
          {allDecided && (<Button size="sm" variant="default" className="h-6 px-3 text-[11px]" onClick={handleApply}>
              Apply
            </Button>)}
        </div>)}

      {/* Empty state — no changes detected (e.g. identical files) */}
      {hunks.length === 0 && originalContent === modifiedContent && (<div className="flex items-center justify-center h-9 px-3 border-t bg-muted/20 text-xs text-muted-foreground">
