...
function getCallSummary(tool, args) {
    const normalized = normalizeTool(tool);
    // ── Core tools ──────────────────────────────────────────
    if (normalized === 'read')
    ...
    if (normalized === 'web') {
        ...
        if (args.urls)
            return `${args.urls.length} URLs`;
        return String(args.query ?? '');
    }
    if (normalized === 'agent')
        return truncate(String(args.description ?? args.prompt ?? ''), 80);
    if (normalized === 'todo') {
        ...
        if (inProgress.length)
            return truncate(inProgress[0].title, 60);
        return `${list.length} task(s)`;
    }
    if (normalized === 'jait') {
        const action = String(args.action ?? '');
        if (action.startsWith('memory.'))
            return `${action}: ${truncate(String(args.query ?? args.content ?? ''), 60)}`;
        if (action.startsWith('cron.'))
            return `${action}: ${truncate(String(args.name ?? args.id ?? ''), 40)}`;
        return action || 'jait';
    }
    // ── Legacy tools ─────────────────────────────────────────
    ...
    if (normalized === 'browser.scroll')
        return `x:${String(args.x ?? 0)} y:${String(args.y ?? 0)}`;
    if (normalized === 'browser.select')
        return `${String(args.selector ?? '')} = ${String(args.value ?? '')}`;
    if (normalized === 'browser.wait')
        return `${String(args.selector ?? '')} (${String(args.timeoutMs ?? 10000)}ms)`;
    if (normalized === 'browser.screenshot')
        return String(args.path ?? 'auto path');
    if (normalized === 'browser.search')
        return String(args.query ?? '');
    if (normalized === 'browser.fetch')
        return String(args.url ?? '');
    if (normalized === 'surfaces.start')
        return `Start ${args.type ?? 'surface'}`;
    if (normalized === 'surfaces.stop')
        return `Stop ${args.surfaceId ?? 'surface'}`;
    if (normalized === 'surfaces.list')
...
function formatSystemInfo(data) {
    const lines = [];
    const gb = (v) => `${v} GB`;
    const maybe = (label, key, fmt) => {
        if (data[key] != null)
            lines.push(`${label}: ${fmt ? fmt(data[key]) : data[key]}`);
    };
    maybe('OS', 'osEdition');
    if (!data.osEdition) {
        const parts = [data.type, data.platform, data.release].filter(Boolean);
        if (parts.length)
            lines.push(`OS: ${parts.join(' ')}`);
    }
    maybe('Host', 'hostname');
    maybe('User', 'user');
    maybe('Arch', 'arch');
...
function formatMemorySaveResult(data) {
    ...
    const lines = [];
    if (id)
        lines.push(`Memory saved: ${id}`);
    if (scope)
        lines.push(`Scope: ${scope}`);
    if (content)
        lines.push(`Content: ${truncate(content, 240)}`);
...
function formatCronAddResult(data) {
    const id = typeof data.id === 'string' ? data.id : '';
    ...
    const lines = [];
    if (id)
        lines.push(`Cron job created: ${id}`);
    if (name)
        lines.push(`Name: ${name}`);
    if (cron)
        lines.push(`Schedule: ${cron}`);
    if (toolName)
...
function formatOutput(result, tool) {
    if (!result)
        return '';
    ...
    if (result.message && result.message !== 'Command executed successfully')
...
function getRunningHint(tool, args) {
    const normalized = normalizeTool(tool);
    if (normalized === 'memory.save')
        return 'Saving memory entry...';
    if (normalized === 'cron.add')
    ...
    if (tool.startsWith('terminal.') || tool === 'execute')
        return 'Command is still running...';
    return 'Tool is still running...';
}
function getTerminalOutcomeBadge(call) {
    if (!normalizeTool(call.tool).startsWith('terminal.'))
        return null;
...
function BrowserSnapshotView({ snapshot }) {
    ...
    const elements = splitIndex >= 0
        ...
        : [];
    return (<div className="space-y-2 rounded-md border bg-muted/20 p-3 text-xs">
      <div>
      ...
      <div>
        ...
        {elements.length ? (<ul className="max-h-40 space-y-1 overflow-auto rounded bg-background p-2 font-mono text-[11px]">
            {elements.map((elementLine, index) => (<li key={`${elementLine}-${index}`} className="break-all">{elementLine}</li>))}
          </ul>) : (<div className="rounded bg-background p-2 text-muted-foreground">No interactive elements found.</div>)}
      </div>
    </div>);
}
function BrowserScreenshotView({ path }) {
    ...
    return (<div className="space-y-2 rounded-md border bg-muted/20 p-3 text-xs">
      ...
      <div className="overflow-hidden rounded border bg-background">
        <img src={src} alt="Browser screenshot" className="max-h-80 w-full cursor-crosshair object-contain" onLoad={() => setLoaded(true)} onError={() => setLoaded(false)} onClick={(event) => {
...
function extractStreamingCommand(streamingArgs) {
    if (!streamingArgs)
        return null;
...
function extractStreamingStringField(streamingArgs, key) {
    if (!streamingArgs)
        return null;
...
function PendingToolLabel({ tool, streamingArgs }) {
    ...
    if (meta && isTerminalTool && command) {
        ...
        return (<span className="inline-flex max-w-full min-w-0 items-center gap-1 rounded-sm border border-blue-500/30 bg-[#0b0f14] px-2 py-0.5 text-[#c9d1d9]">
...
function PendingToolBody({ tool, streamingArgs, scrollRef }) {
    const normalized = normalizeTool(tool);
    const isTerminalTool = normalized.startsWith('terminal.');
    const command = isTerminalTool ? extractStreamingCommand(streamingArgs) : null;
    // Terminal with partial command — show the command being built (no raw JSON)
    if (isTerminalTool && command) {
        return (<pre ref={scrollRef} className={cn('text-xs font-mono leading-5 rounded-md px-3 py-2 overflow-x-auto max-h-64 overflow-y-auto whitespace-pre-wrap break-all', 'bg-[#1e1e1e] text-[#cccccc] dark:bg-[#0d1117] dark:text-[#c9d1d9]')}>
    ...
    if (normalized === 'memory.save') {
        ...
        return (<div className={cn('rounded-md border px-3 py-2 text-xs', 'bg-[#1e1e1e] text-[#cccccc] dark:bg-[#0d1117] dark:text-[#c9d1d9]')}>
    ...
    if (normalized === 'cron.add') {
        ...
        {(cron || toolName) && (<div className="mt-1 opacity-80">
            {cron ? `schedule ${cron}` : ''}
            {cron && toolName ? ' • ' : ''}
    ...
    if (streamingArgs) {
        return (<pre ref={scrollRef} className={cn('text-xs font-mono leading-5 rounded-md px-3 py-2 overflow-x-auto max-h-36 overflow-y-auto whitespace-pre-wrap break-all', 'bg-[#1e1e1e] text-[#8b949e] dark:bg-[#0d1117] dark:text-[#8b949e]')}>
        {streamingArgs}
        <span className="inline-block w-1.5 h-3.5 bg-blue-400 animate-pulse ml-0.5 align-text-bottom"/>
      </pre>);
    }
...
export function ToolCallCard({ call, onOpenTerminal }) {
    ...
    return (<Collapsible open={open} onOpenChange={setOpen}>
      <div className="group flex items-center gap-2 rounded-md px-3 py-2 hover:bg-muted/50 transition-colors">
        <CollapsibleTrigger className="flex min-w-0 flex-1 items-center gap-2 text-left">
          <ChevronRight className={cn('h-3.5 w-3.5 shrink-0 text-muted-foreground transition-transform duration-200', open && 'rotate-90')}/>
          <StatusIcon className={cn('h-4 w-4 shrink-0', statusColor, (call.status === 'running' || call.status === 'pending') && 'animate-spin')}/>
          <Icon className={cn('h-4 w-4 shrink-0', meta.color)}/>
          <span className="text-sm font-medium text-muted-foreground truncate flex-1">
            {isPending ? (<PendingToolLabel tool={call.tool} streamingArgs={call.streamingArgs}/>) : isTerminal ? (<span className="inline-flex max-w-full min-w-0 items-center gap-1 rounded-sm border border-zinc-700/60 bg-[#0b0f14] px-2 py-0.5 text-[#c9d1d9]">
                <span className="shrink-0 text-[10px] text-emerald-400">$</span>
                <code className="min-w-0 truncate text-xs font-mono" title={summary}>{summary}</code>
              </span>) : (<span>{meta.label}: <code className="text-xs font-mono">{summary}</code></span>)}
          </span>
          <span className="text-[11px] text-muted-foreground/60 tabular-nums shrink-0">
            <ElapsedLabel startedAt={call.startedAt} completedAt={call.completedAt} now={now}/>
          </span>
          {terminalOutcomeBadge && (<span className={cn('rounded border px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide shrink-0', terminalOutcomeBadge.className)}>
              {terminalOutcomeBadge.label}
            </span>)}
        </CollapsibleTrigger>
        {canOpenTerminal && onOpenTerminal && (<Tooltip>
      ...
      <CollapsibleContent>
        <div className="ml-[3.25rem] mr-3 mb-2">
          {isPending ? (<PendingToolBody tool={call.tool} streamingArgs={call.streamingArgs} scrollRef={argsScrollRef}/>) : isTerminal ? (<pre ref={terminalScrollRef} className={cn('text-xs font-mono leading-5 rounded-md px-3 py-2 overflow-x-auto max-h-64 overflow-y-auto whitespace-pre-wrap break-all', 'bg-[#1e1e1e] text-[#cccccc] dark:bg-[#0d1117] dark:text-[#c9d1d9]', call.result && !call.result.ok && 'text-red-400 dark:text-red-400')}>
              <span className="text-emerald-400">$ </span>
              {summary}
              {(displayOutput || call.status === 'running') && '\n'}
              {displayOutput}
              {call.status === 'running' && (<span className="inline-block w-1.5 h-3.5 bg-[#cccccc] dark:bg-[#c9d1d9] animate-pulse ml-0.5 align-text-bottom"/>)}
            </pre>) : snapshotText && normalizedTool === 'browser.snapshot' ? (<BrowserSnapshotView snapshot={snapshotText}/>) : screenshotPath ? (<BrowserScreenshotView path={screenshotPath}/>) : isFileEdit && call.status === 'success' ? (<EditDiffView filePath={String(call.args.path ?? '')} oldText={normalizedTool === 'file.patch' ? String(call.args.search ?? '') : undefined} newText={normalizedTool === 'file.patch' ? String(call.args.replace ?? '') : undefined} writtenContent={normalizedTool === 'file.write' ? String(call.args.content ?? '') : undefined} isNewFile={normalizedTool === 'file.write'}/>) : displayOutput ? (<pre className={cn('text-xs font-mono leading-5 rounded-md px-3 py-2 overflow-x-auto max-h-64 overflow-y-auto whitespace-pre-wrap break-all', 'bg-[#1e1e1e] text-[#cccccc] dark:bg-[#0d1117] dark:text-[#c9d1d9]', call.result && !call.result.ok && 'text-red-400 dark:text-red-400')}>
              {displayOutput}
              {call.status === 'running' && (<span className="inline-block w-1.5 h-3.5 bg-[#cccccc] dark:bg-[#c9d1d9] animate-pulse ml-0.5 align-text-bottom"/>)}
            </pre>) : call.status === 'running' ? (<div className={cn('rounded-md border px-3 py-2 text-xs', 'bg-[#1e1e1e] text-[#cccccc] dark:bg-[#0d1117] dark:text-[#c9d1d9]')}>
              <div className="flex items-center gap-2">
...
export function ToolCallGroup({ calls, onOpenTerminal }) {
    if (calls.length === 0)
        return null;
    return (<div className="rounded-lg border bg-card my-2 divide-y">
      {calls.map((call) => (<ToolCallCard key={call.callId} call={call} onOpenTerminal={onOpenTerminal}/>))}
    </div>);
}
