...
export function CliModelSelector({ provider, model, onChange, disabled, className }) {
    const [models, setModels] = useState([]);
    const [loading, setLoading] = useState(false);
    const fetchedProvider = useRef(null);
    useEffect(() => {
        if (provider === 'jait')
            return;
    ...
    return (<DropdownMenu>
      <DropdownMenuTrigger asChild disabled={disabled || loading}>
        <button type="button" className={cn('flex items-center gap-1 rounded-md px-1.5 py-1 text-xs font-medium text-muted-foreground', 'hover:text-foreground hover:bg-muted/60 transition-colors', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring', 'disabled:pointer-events-none disabled:opacity-50', className)} title={`Model: ${model ?? 'none'}`}>
          {loading
      ...
      <DropdownMenuContent align="start" side="top" className="w-64">
        ...
        {models.map((m) => (<DropdownMenuItem key={m.id} onClick={() => onChange(m.id)} className="flex items-start gap-2 py-1.5 cursor-pointer">
            <div className="flex-1 min-w-0">
              <div className="text-sm font-medium">
                {m.name}
                {m.isDefault && (<span className="ml-1.5 text-[10px] text-muted-foreground font-normal">(default)</span>)}
              </div>
              {m.description && (<div className="text-[11px] text-muted-foreground leading-snug">{m.description}</div>)}
            </div>
            {model === m.id && <Check className="h-3.5 w-3.5 mt-0.5 text-primary shrink-0"/>}
          </DropdownMenuItem>))}
        {models.length === 0 && !loading && (<div className="px-2 py-1.5 text-xs text-muted-foreground">No models available</div>)}
      </DropdownMenuContent>
    </DropdownMenu>);
}
//# sourceMappingURL=cli-model-selector.js.map