...
export function EditDiffView({ filePath, oldText, newText, writtenContent, isNewFile, className, }) {
    ...
    if (diffLines.length === 0)
        return null;
    return (<div className={cn('rounded-md border overflow-hidden', className)}>
      {/* File header */}
      ...
      <div className="overflow-x-auto max-h-80 overflow-y-auto">
        <table className="w-full text-xs font-mono leading-5 border-collapse">
          <tbody>
            {diffLines.map((line, i) => (<tr key={i} className={cn(line.type === 'add' && 'bg-green-500/10', line.type === 'remove' && 'bg-red-500/10', line.type === 'header' && 'bg-blue-500/10')}>
                {/* Line numbers */}
                ...
                {/* Gutter symbol */}
                <td className={cn('px-1 w-4 select-none text-center align-top', line.type === 'add' && 'text-green-500', line.type === 'remove' && 'text-red-500', line.type === 'header' && 'text-blue-500')}>
                  {line.type === 'add'
                ? '+'
                : line.type === 'remove'
                    ? '−'
                ...
                <td className="px-2 whitespace-pre-wrap break-all">
                  {line.type === 'header' ? (<span className="text-blue-500/80 italic">{line.content}</span>) : (<span className={cn(line.type === 'add' && 'text-green-600 dark:text-green-400', line.type === 'remove' && 'text-red-600 dark:text-red-400 line-through opacity-70')}>
                      {line.content || '\u00A0'}
                    </span>)}
                </td>
              </tr>))}
...
//# sourceMappingURL=edit-diff-view.js.map