...
const OpenAIIcon = ({ className }) => <OpenAI size={16} className={className}/>;
const ClaudeIcon = ({ className }) => <Claude size={16} className={className}/>;
const PROVIDER_DEFS = [
    {
        value: 'jait',
        label: 'Jait',
...
export function ProviderSelector({ provider, onChange, disabled, className }) {
    ...
    return (<DropdownMenu>
      <DropdownMenuTrigger asChild disabled={disabled}>
        <button type="button" className={cn('flex items-center gap-1 rounded-md px-1.5 py-1 text-xs font-medium text-muted-foreground', 'hover:text-foreground hover:bg-muted/60 transition-colors', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring', 'disabled:pointer-events-none disabled:opacity-50', className)} title={`Provider: ${current.label}`}>
      ...
      <DropdownMenuContent align="start" side="top" className="w-72">
        {PROVIDER_DEFS.map((p) => {
            ...
            return (<DropdownMenuItem key={p.value} onClick={() => onChange(p.value)} disabled={!isAvailable} className="flex items-start gap-2.5 py-2 cursor-pointer">
              <Icon className="h-4 w-4 mt-0.5 shrink-0"/>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium flex items-center gap-1.5">
                  {p.label}
                  {!isAvailable && (<span className="text-[10px] text-muted-foreground">(unavailable)</span>)}
                </div>
