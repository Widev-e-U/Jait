...
function createChipNode(file, onRemove) {
...
function getVsIconName(filename) {
...
/* Component                                                           */
/* ------------------------------------------------------------------ */
export const PromptInput = forwardRef(function PromptInput({ value, onChange, onSubmit, onStop, onQueue, isLoading, disabled, placeholder = 'Ask anything...', className, onVoiceInput, viewMode, onViewModeChange, mode, onModeChange, provider, onProviderChange, cliModel, onCliModelChange, availableFiles = EMPTY_FILES, onSearchFiles, workspaceOpen = false, }, ref) {
    ...
    const handleRemoveChip = useCallback((path) => {
    ...
    useEffect(() => {
        ...
        const alreadyReferenced = new Set(editableRef.current ? getChipPaths(editableRef.current) : []);
        ...
        const instant = currentFiles
            .filter((f) => {
            if (alreadyReferenced.has(f.path))
                return false;
        ...
        const timer = setTimeout(async () => {
            try {
                ...
                for (const r of results) {
                    if (alreadyReferenced.has(r.path) || seen.has(r.path))
                        continue;
                    seen.add(r.path);
                    merged.push(r);
                }
        ...
        const handleNativeInput = () => {
            ...
            if (node.nodeType !== Node.TEXT_NODE) {
                ...
                if (childAtCursor?.nodeType === Node.TEXT_NODE) {
                    node = childAtCursor;
                    cursor = childAtCursor.length;
                }
                else if (cursor > 0 && node.childNodes[cursor - 1]) {
                    let walk = node.childNodes[cursor - 1];
    ...
    const handleKeyDown = useCallback((e) => {
        if (mentionOpen) {
            ...
            if (e.key === 'Enter' || e.key === 'Tab') {
                e.preventDefault();
                if (searchResults[mentionIndex])
                    insertMention(searchResults[mentionIndex]);
                return;
            }
            if (e.key === 'Escape') {
                e.preventDefault();
        ...
        if (e.key === 'Enter' && !e.shiftKey) {
            ...
            if ((text || chips.length > 0) && !isLoading)
                onSubmit(chips);
        }
    ...
    const onDrop = useCallback((e) => {
        ...
        if (jaitFile) {
            try {
                const file = JSON.parse(jaitFile);
    ...
    return (<div className={cn('relative flex flex-col rounded-2xl border bg-background shadow-sm transition-colors focus-within:border-primary/50 focus-within:ring-2 focus-within:ring-primary/20', dragging && 'ring-2 ring-primary/30 border-primary/40', className)} onDragEnter={onDragEnter} onDragLeave={onDragLeave} onDragOver={onDragOver} onDrop={onDrop}>
      {/* Editable area with inline file chips */}
      <div className="relative px-3 pt-3 pb-1.5">
        {isEmpty && (<div className="absolute top-5 left-5 pointer-events-none text-muted-foreground text-base leading-relaxed select-none">
            {placeholder}
          </div>)}
        <div ref={editableRef} contentEditable={!disabled} role="textbox" aria-multiline suppressContentEditableWarning onKeyDown={handleKeyDown} onPaste={handlePaste} className={cn('min-h-[40px] max-h-[200px] overflow-y-auto text-base leading-relaxed outline-none py-2 px-2', 'whitespace-pre-wrap break-words', disabled && 'cursor-not-allowed opacity-50')} style={{ wordBreak: 'break-word' }}/>
      </div>

      {/* @ mention popup */}
      {mentionOpen && (searchResults.length > 0 || searchLoading) && (<div ref={menuRef} className="absolute left-3 bottom-full mb-1 w-72 max-h-52 overflow-y-auto rounded-lg border bg-popover p-1 shadow-lg z-50">
          {searchResults.map((file, i) => (<button key={file.path} type="button" className={cn('flex items-center gap-2 w-full text-left text-xs px-2 py-1.5 rounded-md transition-colors', i === mentionIndex ? 'bg-accent text-accent-foreground' : 'hover:bg-muted')} onMouseDown={(e) => {
                    e.preventDefault();
                    insertMention(file);
          ...
          {searchLoading && (<div className="flex items-center gap-2 px-2 py-1.5 text-xs text-muted-foreground">
              <span className="h-3 w-3 animate-spin rounded-full border-2 border-current border-t-transparent"/>
              Searching...
            </div>)}
          {!searchLoading && searchResults.length === 0 && (<div className="text-xs text-muted-foreground px-2 py-1.5">No matching files</div>)}
        </div>)}

      <div className="flex items-center justify-between px-3 pb-2.5 pt-0.5">
        <div className="flex items-center gap-1">
          {viewMode && onViewModeChange && (<ViewModeSelector mode={viewMode} onChange={onViewModeChange} disabled={disabled || isLoading}/>)}
          {provider && onProviderChange && (<ProviderSelector provider={provider} onChange={onProviderChange} disabled={disabled || isLoading}/>)}
          {provider && provider !== 'jait' && onCliModelChange && (<CliModelSelector provider={provider} model={cliModel ?? null} onChange={onCliModelChange} disabled={disabled || isLoading}/>)}
          {mode && onModeChange && (!provider || provider === 'jait') && viewMode !== 'manager' && (<ModeSelector mode={mode} onChange={onModeChange} disabled={disabled || isLoading}/>)}
        </div>
        <div className="flex items-center gap-1.5">
          {onVoiceInput && !isLoading && (<Button type="button" size="icon" variant="ghost" className="h-8 w-8 shrink-0 rounded-lg" onClick={onVoiceInput} title="Voice input">
              <Mic className="h-4 w-4"/>
            </Button>)}
          {isLoading && (<Button type="button" size="icon" variant="outline" className="h-8 w-8 shrink-0 rounded-lg" onClick={onStop} title="Stop generating">
              <Square className="h-3.5 w-3.5 fill-current"/>
            </Button>)}
          {/* Split send button: primary action + dropdown with Send / Queue */}
          {isLoading && onQueue ? (<div className="flex items-center">
              <Button type="button" size="icon" className="h-8 w-8 shrink-0 rounded-lg rounded-r-none border-r-0" disabled={isEmpty || disabled} title="Queue message" onClick={() => {
                const el = editableRef.current;
                const chips = el ? getChipFiles(el) : [];
                onQueue(chips);
            }}>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button type="button" size="icon" className="h-8 w-5 shrink-0 rounded-lg rounded-l-none px-0" disabled={isEmpty || disabled}>
                    <ChevronDown className="h-3 w-3"/>
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" side="top" className="min-w-[160px]">
                  <DropdownMenuItem disabled={isEmpty || disabled} onSelect={() => {
                const el = editableRef.current;
                const chips = el ? getChipFiles(el) : [];
                onSubmit(chips);
            }}>
            ...
            </div>) : !isLoading ? (<Button type="button" size="icon" className="h-8 w-8 shrink-0 rounded-lg" disabled={isEmpty || disabled} onClick={() => {
                const el = editableRef.current;
                const chips = el ? getChipFiles(el) : [];
                onSubmit(chips);
            }}>
