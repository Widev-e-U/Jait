...
import { X, Play, ChevronDown, ChevronRight, AlertCircle, CheckCircle2, Clock, Loader2, } from 'lucide-react';
...
function ActionItem({ action, expanded, onToggle }) {
    ...
    return (<div className="border rounded-md">
      <button type="button" className="flex items-center gap-3 w-full px-3 py-2.5 text-left hover:bg-muted/50 transition-colors" onClick={onToggle}>
        <StatusIcon className={cn('h-4 w-4 shrink-0', config.color, action.status === 'approved' && 'animate-spin')}/>
      ...
      {expanded && (<div className="px-3 pb-3 pt-1 border-t bg-muted/30">
          <p className="text-sm text-muted-foreground mb-2">{action.description}</p>
          <pre className="text-xs bg-background rounded p-2 overflow-x-auto max-h-48">
            {JSON.stringify(action.args, null, 2)}
          </pre>
          {action.result && (<div className={cn('mt-2 text-xs rounded p-2', action.result.ok ? 'bg-green-500/10 text-green-700 dark:text-green-400' : 'bg-destructive/10 text-destructive')}>
              {action.result.message}
            </div>)}
        </div>)}
    </div>);
}
export function PlanReview({ plan, onApprove, onReject, isExecuting, className }) {
    ...
    return (<Card className={cn('p-4 space-y-3', className)}>
      <div className="flex items-start justify-between gap-3">
        ...
        {isComplete && (<Badge variant={failedCount > 0 ? 'destructive' : 'success'} className="shrink-0">
            {failedCount > 0 ? `${failedCount} failed` : 'Complete'}
          </Badge>)}
      </div>

      <div className="space-y-1.5">
        {plan.actions.map((action) => (<ActionItem key={action.id} action={action} expanded={expandedActions.has(action.id)} onToggle={() => toggleAction(action.id)}/>))}
      </div>

      {pendingCount > 0 && (<div className="flex items-center gap-2 pt-1">
          ...
          <Button size="sm" variant="outline" onClick={onReject} disabled={isExecuting}>
            <X className="h-3.5 w-3.5 mr-1.5"/>
            Reject
          </Button>
        </div>)}

      {isComplete && !isExecuting && (<p className="text-xs text-muted-foreground">
