...
import { Plus, Play, Square, Trash2, Send, Bot, Loader2, CheckCircle2, XCircle, Circle, Pause, RefreshCw, } from 'lucide-react';
...
function StatusPill({ status }) {
    ...
    const Icon = cfg.icon;
    return (<Badge variant="outline" className="flex items-center gap-1 text-xs px-1.5 py-0.5">
...
export function AgentsPage() {
    ...
    const [newProvider, setNewProvider] = useState('jait');
    ...
    const handleStart = async (id, message) => {
        try {
    ...
    const handleSend = async () => {
        if (!selectedId || !sendMessage.trim())
            return;
        try {
    ...
    return (<div className="flex h-full">
      ...
      <div className="w-72 border-r flex flex-col bg-muted/30">
        <div className="p-3 border-b flex items-center justify-between">
          ...
          <Button size="sm" variant="ghost" onClick={() => setShowNew(!showNew)} title="New thread">
            <Plus className="w-4 h-4"/>
          </Button>
        </div>

        {/* New thread form */}
        {showNew && (<div className="p-3 border-b space-y-2">
            <Input placeholder="Thread title..." value={newTitle} onChange={(e) => setNewTitle(e.target.value)} className="h-8 text-sm" autoFocus/>
            <Select value={newProvider} onValueChange={(v) => setNewProvider(v)}>
              <SelectTrigger className="h-8 text-sm">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {providers.map((p) => (<SelectItem key={p.id} value={p.id} disabled={!p.available}>
            ...
            <Textarea placeholder="Initial prompt (optional)..." value={newPrompt} onChange={(e) => setNewPrompt(e.target.value)} className="text-sm min-h-[60px]" rows={2}/>
        ...
        <ScrollArea className="flex-1">
          {threads.length === 0 ? (<div className="p-4 text-sm text-muted-foreground text-center">
              No agent threads yet.
              <br />
              Click + to create one.
            </div>) : (<div className="p-1">
              {threads.map((thread) => (<button key={thread.id} onClick={() => setSelectedId(thread.id)} className={`w-full text-left px-3 py-2 rounded-md text-sm transition-colors ${selectedId === thread.id
      ...
      <div className="flex-1 flex flex-col">
        {error && (<div className="p-2 bg-destructive/10 text-destructive text-sm border-b">
            {error}
        ...
        {!selected ? (<div className="flex-1 flex items-center justify-center text-muted-foreground text-sm">
            ...
            <div className="p-4 border-b flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                {selected.providerSessionId && selected.status !== 'running' && (<Button size="sm" onClick={() => handleStart(selected.id)} className="gap-1">
                    <Play className="w-3 h-3"/> Resume
                  </Button>)}
                {selected.status === 'running' && (<Button size="sm" variant="destructive" onClick={() => handleStop(selected.id)} className="gap-1">
                    <Square className="w-3 h-3"/> Stop
                  </Button>)}
                {(selected.status === 'completed' || selected.status === 'error' || selected.status === 'interrupted') && (<Button size="sm" variant="outline" onClick={() => handleStart(selected.id)} className="gap-1">
                    <RefreshCw className="w-3 h-3"/> Restart
                  </Button>)}
                <Button size="sm" variant="ghost" onClick={() => handleDelete(selected.id)} className="text-destructive">
            ...
            {/* Error banner */}
            {selected.error && (<div className="px-4 py-2 bg-destructive/10 text-destructive text-sm">
                {selected.error}
              </div>)}

            {/* Activity feed */}
            <ScrollArea className="flex-1 p-4">
              {activities.length === 0 ? (<div className="text-sm text-muted-foreground text-center py-8">
                  No activity yet. Start the thread to begin.
                </div>) : (<div className="space-y-2">
                  {activities
                    .slice()
                    .reverse()
                    .map((act) => (<ActivityItem key={act.id} activity={act}/>))}
                  <div ref={activityEndRef}/>
                </div>)}
            </ScrollArea>

            {/* Prompt input (for threads with a live session) */}
            {(selected.status === 'running' || selected.providerSessionId) && (<div className="p-3 border-t flex gap-2">
                <Input placeholder="Send a follow-up message..." value={sendMessage} onChange={(e) => setSendMessage(e.target.value)} onKeyDown={(e) => {
            ...
            {!selected.providerSessionId && selected.status !== 'running' && (<div className="p-3 border-t">
                <div className="flex gap-2">
                  <Input placeholder="Enter a prompt and start the agent..." value={sendMessage} onChange={(e) => setSendMessage(e.target.value)} onKeyDown={(e) => {
                    if (e.key === 'Enter' && !e.shiftKey) {
                        e.preventDefault();
...
function ActivityItem({ activity }) {
    ...
    return (<div className={`text-sm ${color} flex gap-2`}>
