 ...
 */
import { AlertCircle, Bot, CheckCircle2, Info, MessageSquare, Shield, User, Wrench, XCircle } from 'lucide-react';
...
export function ActivityItem({ activity }) {
    ...
    return (<div className={`rounded-lg border p-3 text-sm ${meta.cardClass}`}>
      <div className="flex items-center justify-between gap-3 mb-1.5">
        <span className="inline-flex min-w-0 items-center gap-1.5 text-xs font-medium text-muted-foreground">
          ...
          {tool && (<code className="rounded bg-muted px-1.5 py-0.5 text-[10px]">{tool}</code>)}
        </span>
      ...
      {argsPreview && (<p className="mt-1 text-xs text-muted-foreground whitespace-pre-wrap break-words">
          {argsPreview}
        </p>)}
      {payload && (<details className="mt-2 rounded-md border bg-muted/30 px-2 py-1">
