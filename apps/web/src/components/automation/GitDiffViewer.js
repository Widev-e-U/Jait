...
export function GitDiffViewer({ cwd, baseBranch, onClose }) {
    ...
    return (<div className="fixed inset-0 z-[100] bg-background/80 backdrop-blur-sm flex items-center justify-center p-4" onKeyDown={handleKeyDown} tabIndex={-1}>
      <div className="bg-popover border rounded-lg shadow-xl w-full max-w-6xl h-[85vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b shrink-0">
          <div className="flex items-center gap-2">
            ...
            {!loading && (<span className="text-xs text-muted-foreground">
        ...
        {loading ? (<div className="flex-1 flex items-center justify-center">
          ...
          </div>) : files.length === 0 ? (<div className="flex-1 flex items-center justify-center text-sm text-muted-foreground">
            No changes detected.
            ...
            <div className="w-56 border-r flex flex-col shrink-0 bg-muted/20">
              ...
              <div className="flex-1 overflow-y-auto">
                {files.map((f, i) => (<button key={f.path} className={cn('flex items-center gap-2 w-full px-3 py-1.5 text-left text-xs hover:bg-accent/50 transition-colors', i === selectedIndex && 'bg-accent')} onClick={() => setSelectedIndex(i)} title={`${f.path} (${statusLabel(f.status)})`}>
            ...
            <div className="flex-1 flex flex-col min-w-0">
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 border-b bg-muted/10 shrink-0">
                ...
                <span className={cn('text-[10px] px-1.5 py-0.5 rounded font-medium', selected?.status === 'A' || selected?.status === '?' ? 'bg-green-500/10 text-green-500' :
              ...
              <div className="flex-1 min-h-0">
                {selected && (<DiffEditor key={selected.path} original={selected.original} modified={selected.modified} language={language} theme="vs-dark" options={{
                    readOnly: true,
...
//# sourceMappingURL=GitDiffViewer.js.map