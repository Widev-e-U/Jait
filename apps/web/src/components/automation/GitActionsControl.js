...
import { ChevronDown, GitCommit, CloudUpload, Github, Loader2, ArrowDownToLine, Eye, } from 'lucide-react';
import { gitApi, buildMenuItems, resolveQuickAction, summarizeGitResult, } from '@/lib/git-api';
...
function QuickActionIcon({ quickAction }) {
...
// ── Main component ───────────────────────────────────────────────────
export function GitActionsControl({ cwd, refreshTrigger }) {
    ...
    // ── Actions ────────────────────────────────────────────────────
    const runStackedAction = useCallback(async (action, opts) => {
        setIsBusy(true);
        setMenuOpen(false);
        const toastId = toast.loading(action === 'commit' ? 'Committing...' :
    ...
    return (<div className="relative inline-flex items-center gap-2">
      {/* View changes button */}
      {gitStatus?.hasWorkingTreeChanges && (<Button variant="ghost" size="sm" className="text-xs gap-1" onClick={() => setDiffOpen(true)}>
      ...
      <div className="flex items-center gap-0.5" ref={menuRef}>
        <Button variant="outline" size="sm" className="rounded-r-none text-xs gap-1" disabled={quickAction.disabled || isBusy} onClick={runQuickAction}>
          {isBusy ? <Loader2 className="h-3.5 w-3.5 animate-spin"/> : <QuickActionIcon quickAction={quickAction}/>}
          {quickAction.label}
        </Button>
        <Button variant="outline" size="sm" className="rounded-l-none border-l-0 px-1.5" disabled={isBusy} onClick={() => { setMenuOpen(!menuOpen); refreshStatus(); }}>
          <ChevronDown className="h-3.5 w-3.5"/>
        </Button>

        {/* Dropdown menu */}
        {menuOpen && (<div className="absolute top-full left-0 mt-1 z-50 w-48 rounded-md border bg-popover p-1 shadow-md">
            {menuItems.map((item) => (<button key={`${item.id}-${item.label}`} className={`flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-sm ${item.disabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-accent cursor-pointer'}`} disabled={item.disabled} onClick={() => handleMenuItemClick(item)}>
      ...
      {commitDialogOpen && (<div className="absolute top-full right-0 mt-1 z-50 w-80 rounded-md border bg-popover p-3 shadow-md space-y-2">
          ...
          {isDefaultBranch && (<p className="text-xs text-yellow-600">Warning: committing to default branch</p>)}
          <Input placeholder="Commit message (optional)" value={commitMessage} onChange={(e) => setCommitMessage(e.target.value)} onKeyDown={(e) => {
      ...
      {/* Monaco diff viewer */}
      {diffOpen && (<GitDiffViewer cwd={cwd} onClose={() => setDiffOpen(false)}/>)}
    </div>);
}
//# sourceMappingURL=GitActionsControl.js.map