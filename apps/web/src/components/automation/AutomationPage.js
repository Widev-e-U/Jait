...
import { Plus, Square, Trash2, Send, Bot, Loader2, CheckCircle2, XCircle, Circle, Pause, RefreshCw, FolderOpen, AlertCircle, MessageSquare, } from 'lucide-react';
...
export function AutomationPage() {
    // Repositories
    const [repositories, setRepositories] = useState(loadRepos);
    ...
    const [selectedProvider, setSelectedProvider] = useState('jait');
    ...
    const showGitActions = useMemo(() => selectedThread != null &&
        selectedRepo != null &&
        (selectedThread.status === 'completed' || Boolean(selectedThread.prUrl)), [selectedThread, selectedRepo]);
    // ── Persistence ──────────────────────────────────────────────────────
    useEffect(() => {
    ...
    const repoThreads = useMemo(() => selectedRepo
        ? threads.filter((t) => t.workingDirectory === selectedRepo.localPath ||
    ...
    const removeRepository = useCallback((id) => {
    ...
    return (<div className="flex h-full">
      ...
      <div className="w-72 border-r flex flex-col bg-background">
        ...
        <div className="p-3 border-b space-y-2">
          <div className="flex items-center justify-between">
            ...
            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => setFolderPickerOpen(true)}>
              <Plus className="h-3.5 w-3.5"/>
            </Button>
          </div>

          {repositories.length === 0 ? (<p className="text-xs text-muted-foreground py-2">No repositories yet.</p>) : (<div className="space-y-1">
              {repositories.map((repo) => (<div key={repo.id} className={`group flex items-center gap-2 rounded-md px-2 py-1.5 cursor-pointer text-sm hover:bg-accent ${selectedRepoId === repo.id ? 'bg-accent' : ''}`} onClick={() => {
                    setSelectedRepoId(repo.id);
                    setSelectedThreadId(null);
                }}>
                  ...
                  <Button variant="ghost" size="icon" className="h-5 w-5 shrink-0 opacity-0 group-hover:opacity-100" onClick={(e) => {
                    e.stopPropagation();
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <ScrollArea className="flex-1">
            {loading ? (<div className="flex items-center justify-center py-8">
              ...
              </div>) : repoThreads.length === 0 ? (<p className="text-xs text-muted-foreground p-3">
                {selectedRepo
                ? 'Type a message to start a new thread.'
                : 'Select a repository first.'}
              </p>) : (<div className="p-1 space-y-0.5">
                {repoThreads.map((thread) => (<div key={thread.id} className={`group flex items-center gap-2 rounded-md px-2 py-1.5 cursor-pointer text-sm hover:bg-accent ${selectedThreadId === thread.id ? 'bg-accent' : ''}`} onClick={() => setSelectedThreadId(thread.id)}>
                    <StatusDot status={thread.status}/>
                    <div className="min-w-0 flex-1">
                      <p className="text-xs truncate">{thread.title.replace(/^\[.*?\]\s*/, '')}</p>
                      <p className="text-[10px] text-muted-foreground">
                        {PROVIDER_LABELS[thread.providerId] ?? thread.providerId}
                        {thread.branch ? ` · ${thread.branch}` : ''}
                      </p>
                    </div>
                    <Button variant="ghost" size="icon" className="h-5 w-5 shrink-0 opacity-0 group-hover:opacity-100 text-destructive" onClick={(e) => {
                    e.stopPropagation();
      ...
      <div className="flex-1 flex flex-col min-h-0">
        {/* Header bar */}
        {selectedRepo && (<div className="border-b flex items-center px-4 py-2 gap-3">
            {/* Thread info */}
            {selectedThread ? (<>
                ...
                {selectedThread.branch && (<Badge variant="outline" className="text-[10px] px-1.5 py-0 font-mono">
                    {selectedThread.branch}
                  </Badge>)}
                {selectedThread.status === 'running' && (<Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => void handleStop(selectedThread.id)}>
                    <Square className="h-3 w-3"/>
                  </Button>)}
                {selectedThread.status !== 'running' && !selectedThread.providerSessionId && (<Button variant="ghost" size="sm" className="h-6 text-xs" onClick={() => setSelectedThreadId(null)}>
                    New
                  </Button>)}
        ...
        {error && (<div className="flex items-center gap-2 text-destructive text-sm bg-destructive/10 px-4 py-2">
        ...
        {/* No repo selected */}
        {!selectedRepo && (<div className="flex-1 flex flex-col items-center justify-center gap-4 text-muted-foreground">
        ...
        {/* Chat area */}
        {selectedRepo && (<div className="flex-1 flex flex-col min-h-0">
            {/* Activity feed (scrollable middle area) */}
            {selectedThread && activities.length > 0 ? (<ScrollArea className="flex-1">
                <div className="mx-auto w-full max-w-3xl px-4 py-4 space-y-3">
                  {activities.map((act) => (<ActivityItem key={act.id} activity={act}/>))}
                  <div ref={activityEndRef}/>
                </div>
              </ScrollArea>) : (<div className="flex-1 flex flex-col items-center justify-center text-muted-foreground">
                {selectedThread ? (<>
            ...
            <div className="border-t bg-background px-4 pt-3 pb-4">
              <div className="mx-auto w-full max-w-3xl">
                <div className="rounded-2xl border bg-card shadow-sm">
                  <Textarea ref={inputRef} placeholder={selectedThread?.status === 'running'
                ? 'Send a follow-up message...'
                : 'Describe what you want to do...'} value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} onKeyDown={handleKeyDown} rows={3} className="border-0 bg-transparent resize-none focus-visible:ring-0 text-sm px-4 pt-3 pb-1" disabled={creating}/>
                  <div className="flex items-center justify-between px-3 pb-2">
                    <div className="flex items-center gap-2">
                      <Select value={selectedProvider} onValueChange={(v) => setSelectedProvider(v)} disabled={!!selectedThread}>
                ...
                .map((p) => (<SelectItem key={p.id} value={p.id}>
                                {PROVIDER_LABELS[p.id] ?? p.id}
                              </SelectItem>))}
                          {providers.filter((p) => p.available).length === 0 && (<SelectItem value="jait">Jait</SelectItem>)}
                        </SelectContent>
                      </Select>
                    </div>

                    <Button size="icon" className="h-8 w-8 rounded-full" onClick={() => void handleSend()} disabled={!inputMessage.trim() || creating}>
                      {creating ? (<Loader2 className="h-4 w-4 animate-spin"/>) : (<Send className="h-4 w-4"/>)}
                    </Button>
                  </div>
                </div>
                {showGitActions && selectedRepo && selectedThread && (<ThreadActions threadId={selectedThread.id} cwd={selectedThread.workingDirectory ?? selectedRepo.localPath} branch={selectedThread.branch} baseBranch={selectedRepo.defaultBranch} threadTitle={selectedThread.title} threadStatus={selectedThread.status}/>)}
              </div>
            </div>
          </div>)}
      </div>
      ...
      <FolderPickerDialog open={folderPickerOpen} onOpenChange={setFolderPickerOpen} onSelect={(path) => void handleFolderSelected(path)}/>
    </div>);
}
// ── Sub-components ─────────────────────────────────────────────────────
function ActivityItem({ activity }) {
    const isUser = activity.kind === 'user_message';
    return (<div className={`rounded-lg border p-3 text-sm ${isUser ? 'bg-primary/5 border-primary/20 ml-12' : 'bg-card mr-12'}`}>
      ...
      {activity.summary && (<pre className="whitespace-pre-wrap text-sm leading-relaxed">{activity.summary}</pre>)}
    </div>);
}
//# sourceMappingURL=AutomationPage.js.map