...
import { Monitor, MonitorOff, Smartphone, Globe, Plug, PlugZap, Users, Wifi, Loader2, Cast, Maximize2, Minimize2, RefreshCw, X, ScreenShare, } from 'lucide-react';
// ---------------------------------------------------------------------------
// Sub-components
// ---------------------------------------------------------------------------
function DeviceIcon({ platform, className }) {
    switch (platform) {
        case 'electron':
...
function DeviceCard({ device, isLocal, isConnected, isLoading, onConnect, }) {
    const canConnect = !isLocal && device.capabilities.includes('screen-share');
    return (<div className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors
      ...
      {canConnect && !isConnected && (<Button size="sm" variant="outline" className="h-7 text-xs px-2.5 shrink-0" onClick={onConnect} disabled={isLoading}>
          {isLoading ? (<Loader2 className="h-3 w-3 animate-spin"/>) : (<>
              <Plug className="h-3 w-3 mr-1"/>
              Connect
            </>)}
        </Button>)}
      {isConnected && (<Badge className="bg-green-500/15 text-green-500 border-green-500/30 shrink-0 text-[10px]">
...
function RemoteScreen({ stream, }) {
    ...
    if (!stream) {
        return (<div className="flex-1 bg-black/20 rounded-lg flex items-center justify-center text-muted-foreground">
    ...
    return (<div ref={containerRef} className="relative flex-1 group bg-black rounded-lg overflow-hidden">
      <video ref={videoRef} autoPlay playsInline muted className="w-full h-full object-contain bg-black"/>
      <button onClick={toggleFullscreen} className="absolute top-2 right-2 p-1.5 rounded bg-black/60 text-white opacity-0 group-hover:opacity-100 transition-opacity">
...
// Main component
// ---------------------------------------------------------------------------
export function ScreenSharePanel({ screenShare }) {
    ...
    return (<div className="flex flex-col h-full overflow-hidden">
      ...
      {screenShare.pendingShareRequest && !isConnected && (<div className="p-3 border-b bg-amber-500/5">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex gap-2 mt-2">
                <Button size="sm" className="h-7 text-xs px-3" onClick={() => screenShare.acceptPendingShare()}>
                  <ScreenShare className="h-3 w-3 mr-1"/>
                  Share Screen
                </Button>
                <Button size="sm" variant="ghost" className="h-7 text-xs px-2 text-muted-foreground" onClick={() => screenShare.rejectPendingShare()}>
                  Decline
                </Button>
              </div>
            </div>
      ...
      {isConnected ? (
        ...
        <div className="flex flex-col flex-1 min-h-0">
          ...
          <div className="flex items-center gap-2 px-3 py-2 border-b bg-muted/20 shrink-0">
            ...
            {session && (<span className="text-[10px] text-muted-foreground">
                {session.transport.routeMode.toUpperCase()} · {session.transport.avgLatencyMs}ms
              </span>)}
            <Button variant="ghost" size="sm" className="h-6 text-xs px-2 text-destructive hover:text-destructive" onClick={() => screenShare.disconnect()}>
          ...
          {screenShare.error && (<div className="mx-2 p-2 rounded-md bg-red-500/10 text-red-500 text-xs border border-red-500/20">
              {screenShare.error}
            </div>)}
          ...
          {screenShare.isHost ? (
            ...
            <div className="flex-1 flex items-center justify-center text-muted-foreground">
              <div className="text-center">
                ...
                {session && (<p className="text-xs mt-2 opacity-50">
            ...
            <>
              ...
              {/* Session details */}
              {session && (<div className="flex items-center gap-3 px-3 py-1.5 border-t text-[10px] text-muted-foreground shrink-0">
        ...
        <div className="flex flex-col gap-3 p-3 overflow-y-auto">
          {/* Error */}
          {screenShare.error && (<div className="p-2.5 rounded-md bg-red-500/10 text-red-500 text-xs border border-red-500/20">
              {screenShare.error}
            </div>)}
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" className="h-5 w-5 p-0" onClick={() => screenShare.refreshState()}>
                <RefreshCw className="h-3 w-3"/>
              </Button>
            </div>

            {remoteDevices.length === 0 ? (<div className="py-6 text-center">
                ...
                {remoteDevices.map((device) => (<DeviceCard key={device.id} device={device} isLocal={false} isConnected={false} isLoading={screenShare.loading && screenShare.connectedDeviceId === device.id} onConnect={() => screenShare.requestRemoteShare(device.id)}/>))}
              </div>)}
          </div>

          {/* This device */}
          {localDevice && (<div className="space-y-1.5">
              <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wider">
                This Device
              </h3>
              <DeviceCard device={localDevice} isLocal isConnected={false} isLoading={false} onConnect={() => { }}/>
            </div>)}
...
//# sourceMappingURL=screen-share-panel.js.map