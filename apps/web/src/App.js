...
import { AlertTriangle, Calendar, Bug, Cast, Code, Eye, FolderTree, FolderOpen, LogOut, MessageSquare, Monitor, Moon, PanelLeftClose, PanelLeftOpen, Plus, RefreshCw, Settings, Sun, Square, Trash2, Terminal as TerminalIcon, Wifi, X, Loader2 as SpinnerIcon, Pause, CheckCircle2, XCircle, Circle, AlertCircle, } from 'lucide-react';
...
import { WorkspacePanel, workspaceLanguageForPath, DiffView } from '@/components/workspace';
...
function TitleSkeleton({ className = '' }) {
    return (<span className={`inline-block rounded bg-muted animate-pulse ${className}`}>
      <span className="invisible">Generating title</span>
    </span>);
}
function ManagerStatusDot({ status }) {
    const map = {
...
function ThreadPrBadge({ prState }) {
    if (!prState)
        return null;
    const label = prState === 'open'
        ? 'PR created'
        : prState === 'merged'
            ? 'PR merged'
            : 'PR closed';
    const className = prState === 'open'
    ...
    return (<Badge variant="outline" className={`text-[9px] px-1 py-0 h-4 ${className}`}>
      {label}
    </Badge>);
}
function App() {
    ...
    const [screenShareChatWidth, setScreenShareChatWidth] = useState(null);
    ...
    const [chatProvider, setChatProvider] = useState(() => localStorage.getItem('chatProvider') || 'jait');
    const [cliModel, setCliModel] = useState(() => localStorage.getItem('cliModel') || null);
    const [viewMode, setViewMode] = useState(() => localStorage.getItem('viewMode') || 'developer');
    ...
    const [activeWorkspaceFileId, setActiveWorkspaceFileId] = useState(null);
    const [availableFilesForMention, setAvailableFilesForMention] = useState([]);
    ...
    const { user, token, settings, isLoading: authLoading, isAuthenticated, login, register, logout, bindSession, updateSettings, clearSessionArchive, } = useAuth();
    const onLoginRequired = useCallback(() => setShowLoginDialog(true), []);
    const { sessions, activeSessionId, createSession, switchSession, archiveSession, fetchSessions } = useSessions(token, onLoginRequired);
    const { messages, isLoading, remainingPrompts, error, hitMaxRounds, pendingPlan, todoList, changedFiles, messageQueue, contextUsage, sendMessage, restartFromMessage, cancelRequest, clearMessages, continueChat, executePlan, rejectPlan, enqueueMessage, dequeueMessage, updateQueueItem, acceptFile, rejectFile, acceptAllFiles, rejectAllFiles, setTodoList, addChangedFile, setChangedFiles, setOnChangedFilesSync, refreshMessages, } = useChat(activeSessionId, token, onLoginRequired);
    ...
    const automationMessages = useMemo(() => activitiesToMessages(automation.activities), [automation.activities]);
    ...
    const [savedWorkspace, setSavedWorkspace] = useSessionState(activeSessionId, 'workspace.panel', token);
    const [savedScreenShare, setSavedScreenShare] = useSessionState(activeSessionId, 'screen-share.panel', token);
    const [savedTerminal, setSavedTerminal] = useSessionState(activeSessionId, 'terminal.panel', token);
    const [savedTodos] = useSessionState(activeSessionId, 'todo_list', token);
    const [savedChangedFiles] = useSessionState(activeSessionId, 'changed_files', token);
    // Restore panel state from REST fallback (only if WS full-state hasn't arrived yet)
    useEffect(() => {
        if (wsFullStateReceivedRef.current)
            return; // WS already delivered authoritative state
        if (!savedWorkspace)
            return;
        if (activeWorkspace)
            return;
    ...
    useEffect(() => {
        ...
        if (savedChangedFiles && savedChangedFiles.length > 0) {
            for (const f of savedChangedFiles)
                addChangedFile(f.path, f.name);
        }
    }, [savedChangedFiles, addChangedFile]);
    // ── Cross-client state sync handler ───────────────────────────────
    const handleStateSync = useCallback((key, value) => {
        switch (key) {
            case 'workspace.panel': {
                if (!value) {
                    ...
                    if (v.open) {
                        ...
                        if (v.remotePath)
                            setActiveWorkspace({ surfaceId: v.surfaceId ?? '', workspaceRoot: v.remotePath });
                    }
                    else {
                        setShowWorkspace(false);
                    }
            ...
            case 'file_changed': {
                const fc = value;
                if (fc?.path)
                    addChangedFile(fc.path, fc.name ?? fc.path.split('/').pop() ?? fc.path);
                break;
            }
            case 'changed_files': {
    ...
    const handleFullState = useCallback((state) => {
        wsFullStateReceivedRef.current = true;
        // Workspace panel
        const wp = state['workspace.panel'];
        if (wp && wp.open) {
            setShowWorkspace(true);
            if (wp.remotePath)
                setActiveWorkspace({ surfaceId: wp.surfaceId ?? '', workspaceRoot: wp.remotePath });
        }
        else {
            setShowWorkspace(false);
            setActiveWorkspace(null);
        ...
        // Screen share panel
        const sp = state['screen-share.panel'];
        if (sp && sp.open !== false) {
            setShowScreenShare(true);
        }
        else {
    ...
    const { sendUIState } = useUICommands({
        ...
        listeners: {
            'workspace.open': useCallback((data) => {
                setShowWorkspace(true);
            ...
            'terminal.focus': useCallback((data) => {
                setCurrentView('chat');
                setShowTerminal(true);
    ...
    useEffect(() => {
        if (cliModel)
            localStorage.setItem('cliModel', cliModel);
        else
    ...
    useEffect(() => {
        ...
        if (token) {
            void updateSettings({ chat_provider: chatProvider });
        }
    ...
    const openRemoteWorkspaceOnGateway = useCallback(async (dirPath) => {
    ...
    useEffect(() => {
        if (!activeWorkspace?.surfaceId || !activeWorkspace.workspaceRoot || !activeSessionId)
            return;
        let cancelled = false;
        (async () => {
            try {
    ...
    const changedPaths = useMemo(() => changedFiles.filter((f) => f.state === 'undecided').map((f) => f.path), [changedFiles]);
    useEffect(() => {
    ...
    const handleThemeModeChange = useCallback(async (next) => {
    ...
    const ensureActiveTerminal = useCallback(async (preferredTerminalId = null) => {
    ...
    const handleOpenTerminalFromToolCall = useCallback(async (terminalId) => {
        setCurrentView('chat');
        openTerminalPanel();
    ...
    const mergeWorkspaceFiles = useCallback((incoming) => {
    ...
    const handleChangedFileClick = useCallback(async (filePath) => {
        try {
            const headers = {};
            if (token)
                headers['Authorization'] = `Bearer ${token}`;
            // Try to fetch the backup (original) content from the gateway
            const backupRes = await fetch(`${API_URL}/api/workspace/backup?path=${encodeURIComponent(filePath)}`, { headers });
            if (backupRes.ok) {
            ...
            const readRes = await fetch(`${API_URL}/api/workspace/read?path=${encodeURIComponent(filePath)}`, { headers });
            if (!readRes.ok)
                return;
    ...
    const handleDiffApply = useCallback(async (resultContent) => {
        if (!activeDiff)
            return;
    ...
    const handleFileDrop = useCallback(async (dropped) => {
...
\`\`\``)
    ...
    const handleEditPreviousMessage = useCallback(async (messageId, newContent, messageIndex, messageFromEnd) => {
    ...
    const handleSaveApiKeys = async (next) => {
        const sanitized = Object.fromEntries(Object.entries(next)
            .map(([k, v]) => [k, v.trim()])
            .filter(([, v]) => v.length > 0));
    ...
    const { showDesktopScreenShare, showMobileScreenShare, mobilePanelHeightClass, } = getScreenShareLayoutState({
        isMobile,
        showScreenShare,
        hasMessages,
    });
      ...
      <div className="fixed inset-0 flex flex-col overflow-hidden safe-top safe-bottom safe-left safe-right">
        <header className="flex items-center h-14 px-2 sm:px-5 border-b shrink-0 gap-1 sm:gap-2">
          ...
          <nav className="flex items-center gap-1 min-w-0 overflow-x-auto scrollbar-none">
            <Button variant={currentView === 'chat' ? 'secondary' : 'ghost'} size="sm" className="h-8 text-xs shrink-0 px-2 sm:px-3" onClick={() => setCurrentView('chat')}>
              <MessageSquare className="h-3.5 w-3.5 sm:mr-1.5"/>
              <span className="hidden sm:inline">Chat</span>
            </Button>
            <Button variant={currentView === 'jobs' ? 'secondary' : 'ghost'} size="sm" className="h-8 text-xs shrink-0 px-2 sm:px-3" onClick={() => setCurrentView('jobs')}>
              <Calendar className="h-3.5 w-3.5 sm:mr-1.5"/>
              <span className="hidden sm:inline">Jobs</span>
            </Button>
            <Button variant={currentView === 'network' ? 'secondary' : 'ghost'} size="sm" className="h-8 text-xs shrink-0 px-2 sm:px-3" onClick={() => setCurrentView('network')}>
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
            ...
            {remainingPrompts !== null && remainingPrompts <= 5 && (<span className="text-xs text-muted-foreground mr-1 sm:mr-2 hidden sm:inline">{remainingPrompts} remaining</span>)}
            ...
            {isAuthenticated ? (<DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  <div className="px-2 py-1.5">
                    ...
                    <div className="flex items-center h-7 w-fit rounded-full border bg-muted/50 p-0.5 mt-1.5">
                      {[['light', Sun], ['system', Monitor], ['dark', Moon]].map(([mode, Icon]) => (<Tooltip key={mode}>
                          <TooltipTrigger asChild>
                            <button onClick={() => { void handleThemeModeChange(mode); }} className={`relative flex items-center justify-center h-6 w-6 rounded-full transition-colors ${themeMode === mode
              ...
              </DropdownMenu>) : (<Button variant="ghost" size="sm" className="h-8 text-xs" onClick={() => setShowLoginDialog(true)}>
                Sign in
              </Button>)}
          </div>
        </header>
        ...
        {currentView === 'chat' && (<div className="flex items-center gap-1 px-2 sm:px-5 h-9 border-b shrink-0 bg-muted/30 overflow-x-auto scrollbar-none">
            {/* Sidebar toggle — label changes per viewMode */}
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant={showSidebar ? 'secondary' : 'ghost'} size="sm" className="h-6 text-[11px] px-2 shrink-0" onClick={() => setShowSidebar(s => !s)}>
                  {showSidebar
            ...
            {viewMode === 'developer' && (<>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant={showTerminal ? 'secondary' : 'ghost'} size="sm" className="h-6 text-[11px] px-2 shrink-0" onClick={() => { void handleToggleTerminal(); }}>
                ...
                <div className="relative flex items-center shrink-0">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant={showWorkspace ? 'secondary' : 'ghost'} size="sm" className="h-6 text-[11px] px-2" onClick={() => { void handleOpenWorkspace(); }}>
                  ...
                  {showCloseWorkspaceConfirm && (<>
                      ...
                      <div ref={closeConfirmRef} className="absolute top-full left-0 mt-1 z-50 w-64 rounded-lg border bg-background shadow-lg p-3 space-y-2">
            ...
            {viewMode === 'developer' && (<Tooltip>
                <TooltipTrigger asChild>
                  <Button variant={showDebugPanel ? 'secondary' : 'ghost'} size="sm" className="h-6 text-[11px] px-2 shrink-0" onClick={() => setShowDebugPanel(d => !d)}>
            ...
            {viewMode === 'developer' && (<Tooltip>
                <TooltipTrigger asChild>
                  <Button variant={showScreenShare ? 'secondary' : 'ghost'} size="sm" className="h-6 text-[11px] px-2 shrink-0" onClick={() => showScreenShare ? closeScreenSharePanel() : openScreenSharePanel()}>
            ...
            {viewMode === 'manager' && (<>
                ...
                {automation.selectedThread ? (<div className="flex items-center gap-2 shrink-0">
                    <ManagerStatusDot status={automation.selectedThread.status}/>
                    {isTitlePending(automation.selectedThread.title) ? (<TitleSkeleton className="text-[11px] h-3.5 w-28"/>) : (<span className="text-[11px] text-muted-foreground truncate max-w-[200px]">
                        {automation.selectedThread.title.replace(/^\[.*?\]\s*/, '')}
                      </span>)}
                    {automation.selectedThread.branch && (<Badge variant="outline" className="text-[9px] px-1 py-0 font-mono">
                        {automation.selectedThread.branch}
                      </Badge>)}
                    {automation.selectedThread.status === 'running' && (<Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => void automation.handleStop(automation.selectedThread.id)}>
                        <Square className="h-2.5 w-2.5"/>
                      </Button>)}
                    {automation.showGitActions && automation.selectedRepo && (<div className="ml-2 shrink-0">
                        <ThreadActions threadId={automation.selectedThread.id} cwd={automation.selectedThread.workingDirectory ?? automation.selectedRepo.localPath} branch={automation.selectedThread.branch} baseBranch={automation.selectedRepo.defaultBranch} threadTitle={automation.selectedThread.title} threadStatus={automation.selectedThread.status} prUrl={automation.selectedThread.prUrl} prState={automation.selectedThread.prState} ghAvailable={automation.ghAvailable}/>
                      </div>)}
                  </div>) : automation.selectedRepo ? (<span className="text-[11px] text-muted-foreground shrink-0">
                    {automation.selectedRepo.name} · {automation.selectedRepo.defaultBranch}
                  </span>) : null}
              </>)}
          </div>)}

        {currentView === 'jobs' ? (<div className="flex-1 overflow-y-auto">
            <JobsPage />
          </div>) : currentView === 'network' ? (<div className="flex-1 overflow-y-auto">
            <NetworkPanel token={token}/>
          </div>) : currentView === 'settings' ? (<div className="flex-1 overflow-y-auto">
            <SettingsPage username={user?.username ?? ''} token={token} apiKeys={settings.api_keys} onSaveApiKeys={handleSaveApiKeys} sttProvider={settings.stt_provider} onSttProviderChange={async (next) => {
                await updateSettings({ stt_provider: next });
            }} onClearArchive={handleClearArchive} activityEvents={activityEvents}/>
          </div>) : (<div className={`flex flex-1 min-h-0 overflow-hidden ${isMobile ? 'flex-col' : ''}`}>
            {showSidebar && (<aside className={`overflow-hidden ${isMobile ? 'h-52 border-b shrink-0' : 'w-56 border-r shrink-0'}`}>
                {viewMode === 'manager' ? (<div className="h-full flex flex-col text-sm">
                    {/* Repositories */}
                    ...
                    <div className="border-b max-h-[40%] overflow-y-auto shrink-0">
                      {automation.repositories.map(repo => (<button key={repo.id} className={`w-full flex items-center gap-2 px-3 py-1.5 text-xs hover:bg-muted transition-colors group ${automation.selectedRepo?.id === repo.id ? 'bg-muted font-medium' : ''}`} onClick={() => automation.setSelectedRepoId(repo.id)}>
                          <FolderOpen className="h-3 w-3 shrink-0"/>
                          <span className="truncate flex-1 text-left">{repo.name}</span>
                          <span role="button" tabIndex={0} className="ml-auto shrink-0 opacity-0 group-hover:opacity-100 hover:text-destructive transition-opacity" onClick={(e) => { e.stopPropagation(); automation.removeRepository(repo.id); }} onKeyDown={(e) => { if (e.key === 'Enter') {
                        e.stopPropagation();
                        automation.removeRepository(repo.id);
                    } }}>
                            <Trash2 className="h-3 w-3"/>
                          </span>
                        </button>))}
                      {automation.repositories.length === 0 && (<button className="w-full px-3 py-4 text-center text-xs text-muted-foreground hover:text-foreground transition-colors" onClick={() => automation.setFolderPickerOpen(true)}>
                          + Add repository
                        </button>)}
                    </div>
                    ...
                    {automation.selectedRepo && (<>
                        ...
                        <div className="flex-1 overflow-y-auto">
                          {automation.repoThreads.map(thread => (<button key={thread.id} className={`w-full flex items-center gap-2 px-3 py-1.5 text-xs hover:bg-muted transition-colors group ${automation.selectedThread?.id === thread.id ? 'bg-muted font-medium' : ''}`} onClick={() => automation.setSelectedThreadId(thread.id)}>
                              <div className="pt-0.5">
                                <ManagerStatusDot status={thread.status}/>
                              </div>
                              <div className="min-w-0 flex-1 text-left">
                                {isTitlePending(thread.title) ? (<TitleSkeleton className="h-3 w-24"/>) : (<span className="truncate block">{thread.title.replace(/^\[.*?\]\s*/, '')}</span>)}
                                <div className="mt-0.5 flex items-center gap-1 min-w-0">
                                  {thread.branch && (<span className="truncate text-[10px] text-muted-foreground font-mono">{thread.branch}</span>)}
                                  <ThreadPrBadge prState={thread.prState}/>
                                </div>
                              </div>
                              <div className="ml-auto flex items-center gap-1 shrink-0">
                                {thread.status === 'running' && (<span role="button" tabIndex={0} className="shrink-0 hover:text-foreground transition-colors" onClick={(e) => { e.stopPropagation(); void automation.handleStop(thread.id); }} onKeyDown={(e) => {
                                    if (e.key === 'Enter') {
                                        e.stopPropagation();
                                ...
                                <span role="button" tabIndex={0} className="shrink-0 opacity-0 group-hover:opacity-100 hover:text-destructive transition-opacity" onClick={(e) => { e.stopPropagation(); void automation.handleDelete(thread.id); }} onKeyDown={(e) => { if (e.key === 'Enter') {
                            e.stopPropagation();
                            void automation.handleDelete(thread.id);
                        } }}>
                                  <Trash2 className="h-3 w-3"/>
                                </span>
                              </div>
                            </button>))}
                          {automation.repoThreads.length === 0 && (<div className="px-3 py-4 text-center text-xs text-muted-foreground">No threads yet</div>)}
                        </div>
                      </>)}
                  </div>) : (<SessionSelector sessions={sessions} activeSessionId={activeSessionId} onSelect={switchSession} onCreate={() => createSession()} onArchive={archiveSession}/>)}
              </aside>)}

            {viewMode === 'developer' && showDesktopWorkspace && !activeDiff && (<WorkspacePanel ref={workspaceRef} autoOpenRemotePath={activeWorkspace?.workspaceRoot ?? null} surfaceId={activeWorkspace?.surfaceId ?? null} files={workspaceFiles} activeFileId={activeWorkspaceFileId} onActiveFileChange={setActiveWorkspaceFileId} onFileDrop={(files) => { void handleFileDrop(files); }} onReferenceFile={(file) => promptInputRef.current?.insertChip({ path: file.path, name: file.name })} onAvailableFilesChange={setAvailableFilesForMention} showTree={showWorkspaceTree} showEditor={showWorkspaceEditor} onToggleTree={toggleWorkspaceTree} onToggleEditor={toggleWorkspaceEditor} changedPaths={changedPaths}/>)}

            {viewMode === 'developer' && showDesktopWorkspace && activeDiff && (<aside className="flex-[4] min-w-0 border-r bg-background overflow-hidden flex flex-col">
                <DiffView filePath={activeDiff.filePath} originalContent={activeDiff.originalContent} modifiedContent={activeDiff.modifiedContent} language={activeDiff.language} onClose={handleDiffClose} onApply={(result) => { void handleDiffApply(result); }}/>
              </aside>)}

            {viewMode === 'developer' && showDesktopScreenShare && (<aside className="flex-[3] min-w-0 border-r bg-background overflow-hidden flex flex-col">
            ...
            {viewMode === 'developer' && showMobileWorkspace && !activeDiff && (showWorkspaceTree || showWorkspaceEditor) && (<section className={`shrink-0 border-b bg-background overflow-hidden ${hasMessages ? 'h-[50dvh] min-h-[220px]' : 'h-[55dvh] min-h-[260px]'}`}>
                <WorkspacePanel ref={workspaceRef} autoOpenRemotePath={activeWorkspace?.workspaceRoot ?? null} surfaceId={activeWorkspace?.surfaceId ?? null} files={workspaceFiles} activeFileId={activeWorkspaceFileId} onActiveFileChange={setActiveWorkspaceFileId} onFileDrop={(files) => { void handleFileDrop(files); }} onReferenceFile={(file) => promptInputRef.current?.insertChip({ path: file.path, name: file.name })} onAvailableFilesChange={setAvailableFilesForMention} showTree={showWorkspaceTree} showEditor={showWorkspaceEditor} onToggleTree={toggleWorkspaceTree} onToggleEditor={toggleWorkspaceEditor} changedPaths={changedPaths} isMobile/>
              </section>)}

            {/* Mobile: sticky show-panel buttons when workspace panels are hidden */}
            {viewMode === 'developer' && showMobileWorkspace && !activeDiff && (!showWorkspaceTree || !showWorkspaceEditor) && (<div className="flex items-center gap-1 px-2 py-1.5 border-b bg-muted/20 shrink-0">
                {!showWorkspaceTree && (<button onClick={showWorkspaceTreePanel} className="flex items-center gap-1 rounded px-2 py-1 text-[11px] text-muted-foreground hover:text-foreground hover:bg-muted transition-colors">
                    <Eye className="h-3.5 w-3.5"/>
                    <FolderTree className="h-3.5 w-3.5"/>
                    {!isMobile && 'Show Files'}
                  </button>)}
                {!showWorkspaceEditor && (<button onClick={showWorkspaceEditorPanel} className="flex items-center gap-1 rounded px-2 py-1 text-[11px] text-muted-foreground hover:text-foreground hover:bg-muted transition-colors">
                    <Eye className="h-3.5 w-3.5"/>
            ...
            {viewMode === 'developer' && showMobileWorkspace && activeDiff && (<section className={`shrink-0 border-b bg-background overflow-hidden ${hasMessages ? 'h-[50dvh] min-h-[220px]' : 'h-[55dvh] min-h-[260px]'}`}>
                <DiffView filePath={activeDiff.filePath} originalContent={activeDiff.originalContent} modifiedContent={activeDiff.modifiedContent} language={activeDiff.language} onClose={handleDiffClose} onApply={(result) => { void handleDiffApply(result); }}/>
              </section>)}

            {viewMode === 'developer' && showMobileScreenShare && (<section className={`shrink-0 border-b bg-background p-2 ${mobilePanelHeightClass}`}>
            ...
            {viewMode === 'developer' && showDesktopScreenShare && (<div className="w-1 shrink-0 cursor-col-resize hover:bg-primary/20 active:bg-primary/30 transition-colors" onMouseDown={onScreenShareDragStart}/>)}

            {viewMode === 'manager' ? (
            /* ── Manager main content ────────────────────────────── */
            <div className="flex-1 min-w-0 flex flex-col min-h-0">
                {!automation.selectedRepo ? (<div className="flex-1 flex flex-col items-center justify-center px-4">
                  ...
                  </div>) : automation.selectedThread ? (<>
                    <Conversation className="min-h-0 flex-1 border-b" compact>
                      {automationMessages.length === 0 && (<div className="text-center text-sm text-muted-foreground py-8">No activity yet</div>)}
                      {automationMessages.map((msg, idx) => (<Message key={msg.id} messageId={msg.id} messageIndex={idx} messageFromEnd={automationMessages.length - 1 - idx} role={msg.role} content={msg.content} toolCalls={msg.toolCalls} segments={msg.segments} isStreaming={automation.selectedThread?.status === 'running' && idx === automationMessages.length - 1} compact/>))}
                    </Conversation>
                    <div className="shrink-0 py-3 px-4">
                      <div className="mx-auto max-w-3xl">
                        {automation.error && (<div className="flex items-center gap-2.5 rounded-lg border border-red-500/40 bg-red-500/10 px-3.5 py-2.5 text-sm text-red-400 mb-2">
                            <AlertTriangle className="h-4 w-4 shrink-0"/>
                            <span className="min-w-0 break-words">{automation.error}</span>
                          </div>)}
                        <PromptInput ref={promptInputRef} value={inputValue} onChange={setInputValue} onSubmit={handleSubmit} onStop={() => { if (automation.selectedThread)
                    void automation.handleStop(automation.selectedThread.id); }} isLoading={automation.selectedThread?.status === 'running'} disabled={automation.creating} placeholder={automation.selectedThread?.providerSessionId || automation.selectedThread?.status === 'running' ? 'Send a follow-up message...' : 'Describe what you want to do...'} viewMode={viewMode} onViewModeChange={setViewMode} provider={chatProvider} onProviderChange={setChatProvider} cliModel={cliModel} onCliModelChange={setCliModel}/>
                        <div className="flex items-center gap-2 px-1 mt-1.5">
                          <button onClick={() => { automation.setSelectedThreadId(null); setInputValue(''); }} className="text-[11px] text-muted-foreground hover:text-foreground transition-colors shrink-0">
                            New thread
                          </button>
                          {automation.selectedThread && automation.selectedThread.status !== 'running' && !automation.selectedThread.providerSessionId && (<span className="text-[11px] text-muted-foreground truncate">
                    ...
                    <div className="w-full max-w-3xl space-y-8">
                      ...
                      {automation.error && (<div className="flex items-center gap-2.5 rounded-lg border border-red-500/40 bg-red-500/10 px-3.5 py-2.5 text-sm text-red-400">
                          <AlertTriangle className="h-4 w-4 shrink-0"/>
                          <span className="min-w-0 break-words">{automation.error}</span>
                        </div>)}
                      <PromptInput ref={promptInputRef} value={inputValue} onChange={setInputValue} onSubmit={handleSubmit} disabled={automation.creating} placeholder="Describe what you want to do..." viewMode={viewMode} onViewModeChange={setViewMode} provider={chatProvider} onProviderChange={setChatProvider} cliModel={cliModel} onCliModelChange={setCliModel}/>
                    </div>
                  </div>)}
              </div>) : !hasMessages ? (<div className={`${showDesktopScreenShare ? (screenShareChatWidth == null ? 'flex-[2]' : '') : 'flex-1'} min-w-0 flex flex-col items-center justify-center px-4 transition-all duration-300 ease-out`} style={showDesktopScreenShare && screenShareChatWidth != null ? { width: screenShareChatWidth, flexShrink: 0 } : undefined}>
                <div className="w-full max-w-3xl space-y-8">
                  <div className="text-center">
                  ...
                  <PromptInput ref={promptInputRef} value={inputValue} onChange={setInputValue} onSubmit={handleSubmit} onStop={cancelRequest} onQueue={handleQueue} isLoading={isLoading} onVoiceInput={handleVoiceInput} mode={chatMode} onModeChange={setChatMode} provider={chatProvider} onProviderChange={setChatProvider} cliModel={cliModel} onCliModelChange={setCliModel} viewMode={viewMode} onViewModeChange={setViewMode} availableFiles={availableFilesForMention} onSearchFiles={handleSearchFiles} workspaceOpen={showWorkspace}/>
                </div>
              </div>) : (<div className={`flex flex-col ${showDesktopScreenShare ? (screenShareChatWidth == null ? 'flex-[2]' : '') : 'flex-1'} min-w-0 min-h-0 transition-all duration-300 ease-out`} style={showDesktopScreenShare && screenShareChatWidth != null ? { width: screenShareChatWidth, flexShrink: 0 } : undefined}>
                {/* Sticky show-panel buttons when workspace panels are hidden */}
                {showWorkspace && (!showWorkspaceTree || !showWorkspaceEditor) && !isMobile && (<div className="flex items-center gap-1 px-2 py-1 border-b bg-muted/20 shrink-0">
                    {!showWorkspaceTree && (<Tooltip>
                        <TooltipTrigger asChild>
                          <button onClick={showWorkspaceTreePanel} className="flex items-center gap-1 rounded px-2 py-0.5 text-[11px] text-muted-foreground hover:text-foreground hover:bg-muted transition-colors">
                    ...
                    {!showWorkspaceEditor && (<Tooltip>
                        <TooltipTrigger asChild>
                          <button onClick={showWorkspaceEditorPanel} className="flex items-center gap-1 rounded px-2 py-0.5 text-[11px] text-muted-foreground hover:text-foreground hover:bg-muted transition-colors">
                            <Eye className="h-3 w-3"/>
                ...
                <Conversation className="min-h-0 flex-1 border-b" compact={showDesktopWorkspace || showDesktopScreenShare}>
                  {messages.map((msg, idx) => (<Message key={msg.id} messageId={msg.id} messageIndex={idx} messageFromEnd={messages.length - 1 - idx} role={msg.role} content={msg.content} displayContent={msg.displayContent} referencedFiles={msg.referencedFiles} thinking={msg.thinking} thinkingDuration={msg.thinkingDuration} toolCalls={msg.toolCalls} segments={msg.segments} isStreaming={isLoading && msg === messages[messages.length - 1]} compact={showWorkspace || showScreenShare} onOpenTerminal={handleOpenTerminalFromToolCall} onEditMessage={handleEditPreviousMessage}/>))}
                </Conversation>
                ...
                <div className={`shrink-0 py-3 ${(showDesktopWorkspace || showDesktopScreenShare) ? 'px-3' : 'px-4'}`}>
                  <div className={`mx-auto space-y-1.5 ${(showDesktopWorkspace || showDesktopScreenShare) ? 'max-w-none' : 'max-w-3xl'}`}>
                    ...
                    {error && error !== 'login_required' && error !== 'limit_reached' && !isLoading && (<div className="flex items-center gap-2.5 rounded-lg border border-red-500/40 bg-red-500/10 px-3.5 py-2.5 text-sm text-red-400 dark:text-red-400 dark:border-red-400/40 dark:bg-red-400/10">
                        <AlertTriangle className="h-4 w-4 shrink-0"/>
                        <span className="min-w-0 break-words">{error}</span>
                      </div>)}
                    {hitMaxRounds && !isLoading && (<div className="flex items-center justify-center gap-2 py-1.5">
                        <button onClick={() => continueChat({ token, sessionId: activeSessionId })} className="inline-flex items-center gap-1.5 rounded-md border bg-background px-3 py-1.5 text-xs font-medium text-foreground shadow-sm hover:bg-accent transition-colors">
                    ...
                    <ConsentQueue compact sessionId={activeSessionId} onApproveAllEnabled={() => setApproveAllInSession(true)}/>
                    {pendingPlan && (<PlanReview plan={pendingPlan} onApprove={executePlan} onReject={rejectPlan} isExecuting={isLoading}/>)}
                    {limitReached && (<p className="text-center text-sm text-destructive">
                        Daily limit reached. Come back tomorrow.
                      </p>)}
                    {changedFiles.length > 0 && (<FilesChanged files={changedFiles} onAccept={acceptFile} onReject={rejectFile} onAcceptAll={acceptAllFiles} onRejectAll={rejectAllFiles} onFileClick={handleChangedFileClick}/>)}
                    {messageQueue.length > 0 && (<MessageQueue items={messageQueue} onRemove={dequeueMessage} onEdit={updateQueueItem}/>)}
                    <PromptInput ref={promptInputRef} value={inputValue} onChange={setInputValue} onSubmit={handleSubmit} onStop={cancelRequest} onQueue={handleQueue} isLoading={isLoading} disabled={limitReached} onVoiceInput={handleVoiceInput} mode={chatMode} onModeChange={setChatMode} provider={chatProvider} onProviderChange={setChatProvider} cliModel={cliModel} onCliModelChange={setCliModel} viewMode={viewMode} onViewModeChange={setViewMode} availableFiles={availableFilesForMention} onSearchFiles={handleSearchFiles} workspaceOpen={showWorkspace}/>
                    <div className="flex items-center justify-between gap-2 px-1">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        {viewMode === 'manager' && (<button onClick={() => automation.setSelectedThreadId(null)} className="text-[11px] text-muted-foreground hover:text-foreground transition-colors shrink-0">
                            New thread
                          </button>)}
                        {approveAllInSession && (<>
                            ...
                            <button onClick={handleClearApproveAll} className="text-[11px] text-muted-foreground hover:text-foreground transition-colors shrink-0">
                              Clear approve all
                            </button>
                          </>)}
                      </div>
                      {remainingPrompts !== null && (<span className="text-[11px] text-muted-foreground shrink-0">{remainingPrompts} remaining</span>)}
                    </div>
                  </div>
                </div>
              </div>)}
        ...
        {showTerminal && currentView === 'chat' && viewMode === 'developer' && (<div className="shrink-0 border-t overflow-hidden" style={{ height: terminalHeight }}>
            <div onMouseDown={handleDragStart} className="h-1 cursor-row-resize hover:bg-primary/30 transition-colors"/>
            <div className="relative">
              <TerminalTabs terminals={terminals} activeTerminalId={activeTerminalId} onSelect={setActiveTerminalId} onCreate={() => createTerminal(activeSessionId ?? 'default')} onKill={handleKillTerminal}/>
              <button onClick={closeTerminalPanel} className="absolute right-2 top-1/2 -translate-y-1/2 p-0.5 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" aria-label="Close terminal">
                <X className="h-3.5 w-3.5"/>
              </button>
            </div>
            {activeTerminalId ? (<TerminalView terminalId={activeTerminalId} className="h-[calc(100%-2rem)]"/>) : (<div className="flex items-center justify-center h-[calc(100%-2rem)] text-sm text-muted-foreground">
                <button onClick={() => createTerminal(activeSessionId ?? 'default')} className="hover:text-foreground transition-colors">
                  + New Terminal
                </button>
              </div>)}
          </div>)}

        {viewMode === 'developer' && showDebugPanel && (<div className="fixed top-14 right-0 bottom-0 w-[420px] border-l z-50 shadow-xl">
              ...
              <TabsContent value="login" className="pt-4">
                <form className="space-y-4" onSubmit={handleLogin}>
                  <div className="space-y-1.5">
                    ...
                    <Input id="login-username" value={loginUsername} onChange={(event) => setLoginUsername(event.target.value)} autoComplete="username" required/>
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="login-password">Password</Label>
                    <Input id="login-password" type="password" value={loginPassword} onChange={(event) => setLoginPassword(event.target.value)} autoComplete="current-password" required/>
                  </div>
              ...
              <TabsContent value="register" className="pt-4">
                <form className="space-y-4" onSubmit={handleRegister}>
                  <div className="space-y-1.5">
                    ...
                    <Input id="register-username" value={registerUsername} onChange={(event) => setRegisterUsername(event.target.value)} autoComplete="username" required/>
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="register-password">Password</Label>
                    <Input id="register-password" type="password" value={registerPassword} onChange={(event) => setRegisterPassword(event.target.value)} autoComplete="new-password" required/>
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="register-password-confirm">Confirm password</Label>
                    <Input id="register-password-confirm" type="password" value={registerPasswordConfirm} onChange={(event) => setRegisterPasswordConfirm(event.target.value)} autoComplete="new-password" required/>
                  </div>
        ...
        <FolderPickerDialog open={folderPickerOpen} onOpenChange={setFolderPickerOpen} onSelect={(path) => {
        ...
        <FolderPickerDialog open={automation.folderPickerOpen} onOpenChange={automation.setFolderPickerOpen} onSelect={(path) => { void automation.handleFolderSelected(path); }}/>
      </div>
    </TooltipProvider>);
}
export default App;
