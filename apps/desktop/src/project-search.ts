...
export {
  ProjectSearchInputError as DesktopProjectSearchInputError,
 ...
 * validated before we get here, so anything left is a genuinely missing binary
 * — but a sync throw must still not escape as an unhandled exception.
 */
...
async function enumerateFallbackFiles(
  ...
  // machine-level problem the caller can neither fix nor route around, which is
 ...
 * describes a machine-level problem the caller cannot fix and does not name the
