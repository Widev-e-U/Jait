//! term — remote interactive PTY sessions (mirror of legacy desktop terminal implementation).
//!
//! The the legacy shell layer uses node-pty to spawn an interactive shell per
...
impl SessionRegistry {
    ...
    /// bookkeeping, mirroring the legacy desktop shell `list` on quit).
    ...
    /// the legacy desktop shell `stopAllTerminals` on app quit: kill the process
