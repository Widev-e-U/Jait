...
//!
//! Source: legacy desktop host, `ipcMain.handle("desktop:fs-op")`
//! cases: stat, read, readBinary, write, list, exists, mkdir, readdir,
//! reveal-in-explorer.
//!
//! Caps mirror the legacy desktop shell handlers:
...
/// `patch` — the legacy shell fs-op case: read the file, replace the first exact
...
pub fn get_roots() -> RootsOut {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // Additional drives are enumerated best-effort via wmic in the legacy shell;
