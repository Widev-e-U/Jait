...
//! and the props injected by the legacy desktop shell preload).
...
pub fn host_version() -> String {
    format!("jait-desktop {}", env!("CARGO_PKG_VERSION"))
}

/// Payload for `desktop:os-query` (mirrors the legacy desktop shell handler's shape).
...
fn os_version() -> String {
    std::fs::read_to_string("/etc/os-release")
        ...
        .unwrap_or_else(|| {
            ...
            // mirroring the legacy shell requires the real OS version, so query it.
...
mod tests {
    ...
    #[test]
    fn platform_names_match_expected() {
