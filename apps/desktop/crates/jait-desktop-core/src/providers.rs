...
//! legacy desktop provider runtime).
//!
//! The the legacy shell layer spawns the provider CLI as a long-lived child process:
//!   - codex:  `codex proto`
...
//! (the legacy shell ships a managed copy under userData/bin). Rust version checks
...
/// bundled fallback layout under ~/.jait/bin (userData/bin in the legacy shell).
...
/// `start-session` param assembly in the legacy desktop shell / remote-agent.ts).
