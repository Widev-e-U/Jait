...
//! slices of `legacy desktop host`).
//!
//! Two child-process engines, mirroring the legacy desktop shell layer:
//!
...
//!   the legacy shell layer notes the positional-arg form overflows the Windows
...
/// "sessionId": "…" }` — the same shape the legacy desktop shell layer emitted.
...
fn drain_stderr(stderr: ChildStderr, tail: StderrTail) -> std::thread::JoinHandle<()> {
    std::thread::spawn(move || {
...
// ── Argv + JSON-RPC bodies (the legacy shell parity) ───────────────────────────────
...
/// session. This mirrors the legacy desktop shell desktop's temporary app-server flow.
...
fn pump_codex(
    ...
    let stderr_thread = drain_stderr(stderr, stderr_tail);
    std::thread::spawn(move || {
        let reader = BufReader::new(stdout);
        for line in reader.lines().map_while(Result::ok) {
            if line.trim().is_empty() {
...
impl RunnerHandle {
    /// Run one user turn to completion. Mirrors the legacy desktop shell `send-turn` op:
    ...
    fn claude_turn(&self, resolver: &dyn CommandResolver, message: &str) -> Result<(), String> {
        ...
        *self.session_pid.lock() = Some(child.id());
        ...
        let stderr_tail: StderrTail = Arc::new(Mutex::new(String::new()));
        let stderr_thread = drain_stderr(stderr, stderr_tail.clone());
    ...
    /// mirroring the legacy desktop shell stop-session op.
    pub fn stop(&self) {
