//! Settings store — port of legacy desktop host settings handlers.
...
impl SettingsStore {
    pub fn new(path: PathBuf) -> Self {
        ...
        // Mirror the legacy shell: load once at boot, keep in memory, save on write.
        if let Ok(raw) = std::fs::read_to_string(&store.path) {
            if let Ok(Value::Object(map)) = serde_json::from_str::<Value>(&raw) {
                *store.cache.write() = map;
            }
        }
        store.adopt_legacy_device_id();
        store
    }

    /// Tauri replaced the legacy shell as the Windows shell, so both must identify as
    ...
    /// which made existing legacy-bound accounts and permissions disappear.
    fn adopt_legacy_device_id(&self) {
        let current = self
            .cache
            .read()
            .get("deviceId")
    ...
    fn flush(&self) -> Result<(), String> {
        ...
        serde_json::to_string_pretty(&*self.cache.read())
            ...
            .and_then(|s| {
                std::fs::write(&self.path, s).map_err(|e| format!("failed to save settings: {e}"))
            })
    }
}
...
mod tests {
    ...
    fn round_trip_and_defaults() {
        ...
        assert_eq!(
            store.get("gatewayUrl"),
            Value::String(DEFAULT_GATEWAY_URL.into())
        );
        assert_eq!(
            store.get("provider"),
            Value::String(DEFAULT_PROVIDER.into())
        );
        ...
        store
            .set("gatewayUrl", Value::String("ws://localhost:8787/ws".into()))
            .unwrap();
    ...
    #[test]
    fn adopts_the_legacy_device_id() {
        ...
        std::fs::write(
            ...
            r#"{ "deviceId": "legacy-existing-device" }"#,
        )
        .unwrap();
        std::fs::write(
        ...
        assert_eq!(store.device_id(), "legacy-existing-device");
        assert_eq!(
            serde_json::from_str::<Value>(
                &std::fs::read_to_string(tauri_dir.join("settings.json")).unwrap()
            )
            .unwrap()["deviceId"],
            Value::String("legacy-existing-device".into())
        );
