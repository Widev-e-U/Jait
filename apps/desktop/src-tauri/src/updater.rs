//! Auto-updater (the legacy shell parity for `initAutoUpdater` + the `update:*` IPC
//! handlers in the legacy desktop shell).
//!
//! Legacy shell (the updater): `autoDownload = false`, background poll at
...
/// (the legacy shell caches the downloaded installer inside autoUpdater until
...
fn emit_update_event(app: &AppHandle, name: &str, payload: Value) {
    ...
    // onUpdateEvent subscribes to; also emit the full the legacy shell channel name
...
async fn check_and_emit(app: &AppHandle) -> Value {
    let updater = match app.updater() {
        Ok(updater) => updater,
        Err(e) => {
            // Unconfigured updater (e.g. dev build) behaves like the legacy desktop shell's
...
/// mirroring the updater's `download-progress` events. The downloaded
/// bytes are cached for `installUpdate` (the legacy shell caches its installer).
#[tauri::command]
        ...
        .download(
            move |chunk, total| {
                *transferred.lock() += chunk as u64;
                // Legacy shell emits every progress tick; setup.exe artifacts are
...
/// `update:install` parity — the legacy shell calls `autoUpdater.quitAndInstall()`
...
/// Background poll — the legacy shell: `setTimeout(check, 10_000)` then
