...
//!
//! The legacy desktop build has shipped for a long time: its NSIS
...
//! bundle writes **no** installer hooks, so a user who migrates from the legacy shell
...
//! is persisted as `launchAtLogin: true` so an the legacy shell uninstaller deleting
...
/// Persisted settings key shared with the legacy desktop shell and the shim.
...
fn read_run_value(app: &AppHandle) -> Option<String> {
    ...
    // name the legacy desktop shell NSIS installer used, so they share one entry.
...
///
/// Compares the full path (case-insensitive): the legacy desktop shell install's exe is
...
/// Adopt the legacy desktop shell-era login item on first Tauri start. No-op afterwards.
///
...
pub fn sync_login_item(app: &AppHandle, glue: &Arc<Mutex<HostState>>) {
    ...
    // for the legacy desktop shell install path and (b) re-creates an entry the legacy desktop shell
