...
use tauri::{
    ...
    AppHandle, Emitter, Manager, RunEvent, State, WebviewUrl, WebviewWindowBuilder, Window,
};
...
use crate::hosting::{
    desktop_gateway_configure, desktop_gateway_restart_app, desktop_gateway_status, DesktopGateway,
};
...
/// First-launch login-item takeover (adopt the legacy desktop shell install's autostart).
pub mod login_item;
...
/// window actually close instead of hiding (the legacy shell `isQuitting` parity).
static QUITTING: AtomicBool = AtomicBool::new(false);

/// Show + focus the main window. the legacy shell parity:
...
/// the legacy desktop shell: Show Jait / Screen Share submenu (Start/Stop Sharing →
...
fn create_tray(app: &AppHandle, glue: &Arc<Mutex<HostState>>) -> tauri::Result<()> {
    ...
    TrayIconBuilder::with_id("jait-tray")
        ...
        .on_tray_icon_event(|tray, event| {
            // Legacy shell: tray.on("click"/"double-click") → show + focus.
...
/// Legacy shell parity: `dialog.showOpenDialog({ properties: ['openDirectory'] })`
...
/// Legacy shell parity: `win.isMaximized() ? win.unmaximize() : win.maximize()`.
///
...
/// Detached "project" window — the legacy shell parity for
...
/// Login-item state — the legacy shell parity for `desktop:get-login-item`
/// (the legacy desktop shell `app.getLoginItemSettings({ args: AUTO_START_ARGS })`).
...
/// Login-item toggle — the legacy shell parity for `desktop:set-login-item`
/// (the legacy desktop shell `app.setLoginItemSettings({ openAtLogin, args })`).
/// The registered launcher keeps the `--hidden` arg from plugin init, so
/// login-time launches minimize to the tray exactly like the legacy shell.
#[tauri::command]
pub fn desktop_set_login_item(app: AppHandle, enabled: bool) -> Result<Value, String> {
    ...
    // the legacy desktop shell's app.setLoginItemSettings has no such quirk, so this keeps
    // `desktop:set-login-item` reliable everywhere.
    if enabled {
        let autostart_dir = dirs::config_dir()
            .ok_or_else(|| "login-item: no user config directory".to_string())?
            .join("autostart");
        std::fs::create_dir_all(&autostart_dir).map_err(|e| format!("login-item: {e}"))?;
    }
...
/// Gateway URL — the legacy shell parity with the legacy desktop shell:
...
/// Renderer URL. the legacy shell dev loads `http://localhost:3000` (vite dev server,
...
/// mirroring the legacy desktop shell / the preload shim contract (gatewayUrl, deviceID).
fn boot_script(
    gateway: &str,
    ...
    let device_id = glue
        ...
        .unwrap_or_default();
    // `openFolder` feeds the shim's initial onOpenFolder callback (the legacy shell:
    ...
    let open_folder_json = match open_folder {
        Some(path) => {
            serde_json::to_string(&path.display().to_string()).unwrap_or_else(|_| "null".into())
        }
        None => "null".to_string(),
    };
    format!(
...
/// Linux window managers already provide reliable dragging, caption controls,
/// resizing, and desktop integration. Keep their native frame instead of
/// depending on WebKitGTK's handling of a synthetic HTML drag region.
fn use_native_window_decorations() -> bool {
    cfg!(target_os = "linux")
}
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .plugin(tauri_plugin_single_instance::init(|app, argv, _cwd| {
            // Legacy shell second-instance handler: focus the main window and
            ...
            if let Some(folder) = crate::resolve_folder_arg(&argv) {
                let _ = app.emit(
                    "open-folder",
                    json!({ "folderPath": folder.display().to_string() }),
                );
            }
        }))
        // Login-item (launch at login) parity for the legacy desktop shell's
        // `desktop:get/set-login-item`. The `--hidden` args match the legacy desktop shell's
        ...
        .setup(move |app| {
            ...
            boot.push_str(&format!(
                "window.__JAIT_DESKTOP_BOOT__.gatewayConfig = {};",
            ...
            let mut builder = WebviewWindowBuilder::new(app, "main", web_url(&gateway))
                ...
                .decorations(use_native_window_decorations());
            ...
            builder
                ...
                .build()?;
            // Legacy shell parity: tray icon + background menu (createTray).
        ...
        // background behavior keeps webview state alive (the legacy shell
        ...
        .run(|app_handle, event| {
            ...
            // the legacy desktop shell-parity gap report.
            if let RunEvent::Exit = event {
                if let Some(host) = app_handle.try_state::<DesktopGateway>() {
                    host.0.lock().stop();
                }
            }
        });
...
mod tests {
    ...
    fn gateway_url_defaults_to_localhost() {
        // Legacy shell: process.env["JAIT_GATEWAY_URL"] ?? "http://localhost:8000"
    ...
    #[test]
    fn linux_uses_native_window_decorations() {
        assert_eq!(use_native_window_decorations(), cfg!(target_os = "linux"));
    }
}
