//! jait-desktop-app — Tauri v2 adapter over the desktop-shell parity glue.
//!
//! Layering (same rule set in `apps/desktop`):
//! - `jait-desktop-core` — raw OS ops, no webview, no IPC surface
//! - `jait-desktop-glue` — the full desktop IPC command surface
//!   (`desktop:fs-op`, `desktop:provider-op`, `desktop:terminal-op`, …)
//! - this crate — Tauri command plumbing + glue→the legacy shell event translation
//!
...
/// Translate a glue sink event into the legacy desktop shell wire shape the renderer
/// consumes. the legacy shell funnels *all* child-process traffic through the single
...
///
/// Contracts mirrored from `legacy desktop host`:
...
///   where `output` is stdout+stderr concatenated (the legacy shell sent one combined string).
pub fn translate_glue_event(channel: &str, payload: &Value) -> Option<(String, Value)> {
    match channel {
        ...
        "terminal:exit" => {
            // Legacy shell reported the raw signal; glue reports an error string.
            let mut out = payload.clone();
            if let Some(obj) = out.as_object_mut() {
                obj.insert("type".into(), json!("terminal.exit-from-child"));
                let signal = obj.get("error").cloned().unwrap_or(Value::Null);
                obj.remove("error");
        ...
        "background:complete" => {
            let mut out = payload.clone();
            if let Some(obj) = out.as_object_mut() {
                obj.insert("type".into(), json!("tool.background-complete-from-child"));
                let stdout = obj
                    .get("stdout")
                    .and_then(Value::as_str)
                    .unwrap_or_default()
...
/// Mirrors the legacy desktop shell's second-instance → `renderer:open-folder` hand-off:
/// the path is canonicalized when it exists so the webview receives the
/// real location, and kept as-is when it doesn't (same as the legacy desktop shell,
...
pub fn resolve_folder_arg(argv: &[String]) -> Option<std::path::PathBuf> {
    ...
    if !path.is_absolute() {
        return None; // same guard the legacy desktop shell applies
    }
...
/// Launch flags shared with the legacy shell: `--hidden` (also `-hidden`/`--start-hidden`,
/// the historical convention) starts minimized to the tray, `--open-folder`
...
mod tests {
    ...
    #[test]
    fn launch_options_legacy_hidden_aliases() {
    ...
    fn resolve_folder_canonicalizes_existing_paths() {
        ...
        let argv: Vec<String> = vec![
            ...
            format!("--open-folder={}", dir.path().join("link-target").display()),
        ];
    ...
    #[test]
    fn provider_events_get_the_legacy_child_envelope() {
        let payload = json!({
            "type": "provider.line",
            "sessionId": "s1",
    ...
    #[test]
    fn serialized_provider_events_get_the_legacy_child_envelope() {
        let payload = serde_json::to_value(jait_desktop_core::ProviderEvent::Line {
            session_id: "s1".into(),
            line: "{\"method\":\"item/agentMessage/delta\",\"params\":{\"delta\":\"hello\"}}"
                .into(),
        })
        .expect("provider event serializes");
    ...
    #[test]
    fn provider_turn_completion_gets_the_legacy_child_envelope() {
        let payload = json!({
            "type": "provider.turn-completed",
            "sessionId": "s1",
        });
        let (_, out) =
            translate_glue_event("gateway:event", &payload).expect("provider completion forwarded");
        assert_eq!(out["type"], "provider.event-from-child");
        assert_eq!(out["sessionId"], "s1");
    ...
    #[test]
    fn terminal_output_gets_legacy_type() {
