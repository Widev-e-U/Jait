 ...
 *
 * Web-facing desktop bridge contract:
 ...
 *
 * Static members: platform ('desktop' — web's
 * detectPlatform() is presence-based) and deviceId (from boot constants).
 *
 * Every desktop IPC handler maps onto the single `desktop_ipc` Tauri command
...
(function () {
  ...
  // Single funnel for all desktop-shell parity IPC. Channel names are validated
  ...
  // the legacy desktop shell's removeGatewayEventListener removes all listeners at once.
  ...
  function detectProviders() {
    return dispatch('desktop:detect-providers', []).then(function (result) {
      // Glue already returns the legacy desktop shell shape ({ providers: [...] }).
      return result;
    });
  }
  ...
  // the legacy desktop shell's desktopCapturer source list has no Tauri equivalent in the
  ...
  function onMaximizedChange(callback) {
    ...
    // Catch up immediately with the current state (deduped like the legacy shell).
  ...
  function openProjectWindow(options) {
    ...
    return invoke('open_project_window', {
      ...
      // Legacy shell resolves { ok: true } and carries the new window label.
  ...
  // Legacy shell parity: these call the Rust updater commands (crate::updater),
  // which mirror the updater's behavior — check returns
  ...
  function checkForUpdate() {
    return invoke('desktop_update_check')
      .then(function (v) {
        // Older shells without the updater commands resolve null; the legacy shell
  ...
  // Legacy shell contract: onUpdateEvent(eventName, callback) subscribes to the
  ...
  // Legacy shell parity for `desktop:get-login-item`: reports
  ...
  // glue settings store like every other the legacy shell setting.
  ...
  function setLoginItem(enabled) {
    ...
    return invoke('desktop_set_login_item', { enabled: on })
      .then(function (res) {
        // Mirror the legacy desktop shell: persist the toggle so the Settings switch
  ...
  window.jaitDesktop = {
    // statics (the legacy shell preload exposes these synchronously; web's
    ...
    // the legacy desktop shell / the preload shim expose the gateway URL synchronously so
    ...
    // the legacy desktop shell's ipcMain handler for both names toggles; alias for parity.
