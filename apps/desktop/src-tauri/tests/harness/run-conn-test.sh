...
# Usage: run-conn-test.sh
set -u
APP=tauri
HARNESS_DIR=/jait/apps/desktop/src-tauri/tests/harness
OUT="$HARNESS_DIR/out"
mkdir -p "$OUT"
: > "$OUT/$APP-report.json"
...
WEBKIT_DISABLE_COMPOSITING_MODE=1 WEBKIT_FORCE_SANDBOX=0 \
  /jait/apps/desktop/src-tauri/target/debug/jait-desktop-app >"$OUT/$APP-app.log" 2>&1 &
APP_PID=$!
